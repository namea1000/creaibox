...
## 4. 사이드바 관리자 접근 제어 (Sidebar Admin Access)

* **설명**: 일반 사용자에게는 보이지 않는 바로가기 메뉴로, 로그인한 세션 유저의 이메일이 `ADMIN_EMAILS` 목록에 포함되어 있을 경우에만 사이드바 최하단(인포센터 아래)에 "관리자 센터" 및 하위 메뉴 바로가기가 동적으로 생성되어 노출됩니다.
