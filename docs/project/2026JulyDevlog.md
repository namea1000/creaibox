...
* **작업 상세**:
  ...
  - **블로그 메인 헤더 설명글 노출 연동 ([`BlogClientWrapper.tsx`](file:///Users/a1234/Local%20Sites/creaibox/src/app/brand/%5Bbrand_id%5D/components/BlogClientWrapper.tsx))**: 사용자가 입력한 `블로그 설명(상세 설명)` 텍스트가 메인 배너 제목(`blogTitle`) 바로 아래 서브 타이틀로 실시간 출력되도록 연결 완료.
