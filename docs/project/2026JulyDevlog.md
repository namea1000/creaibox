...
* **작업 상세**:
  - **작가/브랜드 프로필 이미지 블로그 구글드라이브 폴더 연동 ([`blog-management/page.tsx`](file:///Users/a1234/Local%20Sites/creaibox/src/app/studio/writing/creaibox/blog-management/page.tsx))**: `sourceType`을 블로그 글 쓸 때와 동일한 **`writing_creaibox_posts`**로 변경하여, 아바타 이미지가 Supabase 버킷이 아닌 사용자의 블로그 구글드라이브 연동 폴더(`writing-creaibox-posts/{YYYYMM}`)에 동일하게 업로드되도록 바인딩 완료.
