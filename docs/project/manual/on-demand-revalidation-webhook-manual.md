...
## 1. 개요 및 목적 (WHY)
이 매뉴얼은 CreaiBox의 블로그, AI 사이트 빌더 등에서 글을 작성하거나 수정했을 때, **0.1초 이내에 라이브 사이트에 즉시 반영(캐시 파기)되고, 백그라운드 웜업 엔진(Warm-up Engine)을 통해 첫 방문자조차 0.01초 만에 본문이 즉시 열리도록 구성**하는 실전 셋업 방법을 안내합니다.

* **타겟 독자**: 기획자, 실무 개발자, DB 관리자
* **적용 대상**: Vercel Global Edge CDN 무한 캐시가 적용된 모든 블로그/사이트 라우트
* **핵심 기능**:
  1. **정밀 타겟 캐시 삭제 (`revalidatePath`)**: 수정된 글의 구형 캐시만 0.01초 만에 파기.
  2. **자동 웜업 핑 (Warm-up Engine)**: 사용자가 방문하기 전에 백그라운드에서 0.1초 만에 미리 호출하여 새 HTML을 Edge CDN에 즉시 구워둠.
