...
### 유튜브 영상 검색 기능 가이드 (`/youtube-trend/search`)
이 메뉴는 사용자가 입력한 특정 키워드를 기반으로 실시간 유튜브 인기 영상을 조회합니다.
- **2-Step 파이프라인 구조**:
  1. 먼저 `search` API를 이용해 최대 24개의 관련 영상 목록(비디오 ID)을 추출합니다. 이 작업에 100 Quota가 소모됩니다.
  2. 추출된 ID를 가지고 `videos` API를 병렬 호출해 조회수, 좋아요 등의 통계 정보를 획득합니다.
- **사용 시 주의점**: 검색 1회 당 최소 101 이상의 Quota가 소모되므로 실시간 호출 방식을 남용하면 일일 할당량(10,000)이 빠르게 고갈될 수 있습니다. `admin_api_vault` 시스템을 통해 할당량이 넉넉한 키로 자동 스위칭하여 장애를 방어합니다.
