...
### ⑩ 🟢 🤖 사이트 100% 무손실 이관을 위한 백그라운드 무인 서브페이지 렌더링 큐 (Sequential Queue Worker)
- **상태**: **🟢 현재 100% 실시간 자동 구동 중** ([`/api/cron/site-migration-worker`](file:///Users/a1234/Local%20Sites/creaibox/src/app/api/cron/site-migration-worker/route.ts))
- **실행 주기**: 1분마다 무인 작동 (Vercel Cron: `* * * * *`)
- **기능**: 사용자가 "전체 사이트 이관"을 실행했을 때, 메인 페이지만 100% 최고 품질(PRO 퀄리티)로 즉시 렌더링을 끝내고 서브페이지 15개는 DB 큐(`extra_configs.migration_queue`)에 적재해 둡니다. 이 백그라운드 워커가 1분마다 깨어나 큐에서 서브페이지 주소를 1개씩 꺼내어, AI 엔진의 모든 출력을 쏟아부어(토큰 제한 방어) 완벽한 퀄리티로 한 장 한 장 백그라운드에서 조립해 나갑니다.
