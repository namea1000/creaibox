...
### 2026-08-19 홈페이지 이관 4대 옵션 체계 확장 & 서브 2차 드롭다운 메뉴 정밀 복제 엔진 탑재 (v1.52) 🟢

- **이관 옵션 4대 체계 확장**: `0. 원페이지 스크롤링 업그레이드`, `1. 원페이지 스크롤링 + 서브 2차 드롭다운 메뉴 복제`, `2. 전체 페이지(15개 미만)`, `3. 전체 페이지(100개 미만)` 완비.
- **0번 모드 (1-Page Smooth Scroll Upgrade)**: 상단 헤더 앵커와 본문 섹션 `id`를 100% 매핑하여 원본 사이트가 부실해도 완성도 높은 1페이지 스크롤링 랜딩페이지로 자동 보완.
- **1번 모드 (2-Tier Dropdown & Mega-Menu Clone)**: Tailwind CSS `group relative` + `group-hover:opacity-100 group-hover:visible` 박스로 원본 헤더의 2차 서브메뉴 완벽 재현.
- **Framer Route Dictionary 전수 파서**: 1,500+ 헤딩/문단 100% 무손실 추출.
- **지능형 JSON 자동 복원 엔진 (`safeParseAiJson`)**: 괄호 누락 및 미세 문법 불일치를 자동 복구하여 섹션 유실 원천 차단.
- **Vertex AI 프롬프트 페이로드 최적화**: 50,000자 + 핵심 미디어 30개 + Framer 키 텍스트 80개 주입으로 5~8개 섹션 100% 온전 생성.
