...
### 2.1 메인 홈 & 헤더 자산 저장소 (Cloudflare R2 영구 스토리지)

* **목적**: 웹사이트 첫 페이지 및 주요 메뉴 로딩 속도 극대화, 원본 사이트 폐쇄 후에도 이미지 영구 보존.
* **저장 위치**: **`CreaiBox Cloudflare R2 버킷 (migrated-sites/{brand_id}/{hash}.webp)`**
* **목적별 지능형 WebP 자동 최적화**:
  - **히어로/풀 배너 (`hero`)**: 최대 너비 `1920px`, 선명한 고화질 WebP (85%)
  - **콘텐츠 카드/그리드 (`card`)**: 최대 너비 `1200px`, 가볍고 빠른 WebP (80%)
  - **로고/아이콘 (`icon`)**: 최대 너비 `512px`, 정밀 WebP (90%)
  - **동영상 (MP4/WEBM/YouTube)**: 원본 스트리밍/임베드 링크를 100% 그대로 유지하여 트래픽 부담 없이 즉시 재생.
* 로고 이미지, 메인 비주얼 썸네일, 헤더 페이지(회사소개, 오시는 길, 서비스 안내) 고화질 자산을 Cloudflare R2 글로벌 엣지 CDN에 전진 배치하여 0.01초 만에 광속 서빙.

### 2.2 내 PC 사진으로 이미지 직접 교체 기능 (Image Manager)

* **메뉴 위치**: 나의 홈페이지 AI 이관 히스토리 카드 ➔ **`[ 🖼️ 내 사진으로 교체 ]`** 버튼
* **기능**:
  - 사이트에 들어있는 모든 이미지(로고, 히어로 배너, 제품 사진, 갤러리)를 썸네일로 한눈에 확인.
  - 바꾸고 싶은 이미지 밑의 **`[ 📁 내 PC 사진으로 변경 ]`**을 누르고 컴퓨터의 사진 파일을 선택하면, 서버에서 목적별 WebP로 자동 압축 변환하여 R2에 업로드하고 사이트 이미지를 실시간 교체.
  - 내 매장의 실제 사진, 공식 로고를 1초 만에 반영하여 100% 맞춤형 브랜드 사이트로 완성 가능.

### 2.3 블로그 포스트 & 원고 저장소

* **목적**: 원고 자산화, 외부 플랫폼 삭제 시 사진 유실 방지, 초고속 0.01초 카드 렌더링.
* **저장 위치**: **`크리에이박스 블로그 ➔ 블로그 원고 관리` (`writing_creaibox_posts`) & `CreaiBox 클라우드 DB (/userId/writing_creaibox_posts/YYYYMM/)`**
* **이중 WebP 저장 파이프라인**:
  1. **본문용 고해상도 WebP**: 1200px 리사이즈, Q82 압축으로 선명한 본문 사진 보존.
  2. **전용 16:9 썸네일 WebP**: 640x360 픽셀, Q78 압축 (~20KB)으로 생성하여 `generated_images` DB에 자동 적재 ➔ 메인 포트폴리오 및 블로그 카드 목록에서 0.01초 만에 로딩.
* **영구 보존**: 타사 플랫폼에서 글이나 사진이 삭제되더라도 CreaiBox Google Cloud DB에 영구 적재되어 있으므로 사진 깨짐(엑박) 0% 보장.
...
## 🧭 22. 홈페이지 이관 4대 옵션 체계 및 서브 2차 드롭다운 복제 실무 가이드 (v1.52)

스튜디오의 이관 입력 폼에서 제공되는 **4대 이관 모드**의 상세 설명 및 선택 가이드입니다:

| 옵션 번호 | 옵션 명칭 | 최적 활용 사례 | 핵심 동작 원리 |
|---|---|---|---|
| **0번** | **0. 메인 페이지 이관 (원페이지(1-Page) 스크롤링 웹사이트)** | 단일 랜딩페이지, 원페이지 소개 사이트, 심플 비즈니스 사이트 | 상단 헤더 메뉴(`Features`, `Services`, `Pricing`, `Contact`)가 본문 섹션으로 즉시 부드럽게 스크롤(Smooth Scroll) 이동하며, **원본 사이트가 부실해도 완성도 높은 1-Page 스크롤링 랜딩페이지로 지능형 자동 업그레이드** |
| **1번** | **1. 메인 페이지 스크롤링 웹사이트 (헤더메뉴 + 서브 2차 메뉴 복제)** | 상단 헤더에 드롭다운 서브메뉴나 메가메뉴가 있는 중대형 사이트 | 1페이지 스크롤링 기반 위에, 원본 헤더의 **마우스 호버 2차 드롭다운 서브메뉴 및 메가메뉴**를 Tailwind CSS `group relative` 호버 드롭다운 박스로 완벽 복제 |
| **2번** | **2. 전체 페이지 이관 (메인+서브페이지 총 15페이지 미만)** | 메인 외에 회사소개, 공지사항 등 서브페이지가 15개 미만인 사이트 | 메인 페이지 즉시 이관 후, 백그라운드 워커가 15개 미만 서브페이지를 큐(`migration_queue`)에 담아 순차 크롤링 및 DB 적재 |
| **3번** | **3. 전체 페이지 이관 (메인+서브페이지 총 100개 미만)** | 포트폴리오, 뉴스, 지점 안내 등 서브페이지가 많은 대형 사이트 | 100개 미만 서브페이지를 대규모 일괄 비동기 파싱 및 병합 |

## 📱 23. 카카오톡 및 SNS 공유 시 독립 OpenGraph 썸네일/설명 노출 매뉴얼 (v1.53)

* **기능 개요**: AI 웹사이트 빌더로 제작된 모든 서브도메인 웹사이트(`000.creaibox.com`)는 이제 링크 공유 시 플랫폼 공용 화면이 아닌 **고객사 웹사이트의 진짜 대표 비주얼(Hero 사진 / WebP 에셋), 상호명, 1줄 소개문**이 담긴 독립된 카카오톡 미리보기 카드로 100% 렌더링됩니다.
* **추출 우선순위**:
  1. **대표 썸네일 이미지 (`og:image`)**: 사이트 첫 섹션에 배치된 메인 비주얼/히어로 이미지(`media_urls[0]` 또는 `content_data.image`)를 1순위로 자동 추출.
  2. **1줄 요약 설명문 (`description`)**: 본문 첫 섹션의 첫 문단(`<p>`) 태그 텍스트에서 15~160자의 핵심 문장을 AI가 깔끔하게 정제하여 카카오톡 요약문으로 노출.
  3. **브랜드 상호명 (`title`)**: 이관된 회사명/브랜드명을 타이틀로 지정.
* **카카오톡 캐시 갱신 팁**:
  - 이미 카카오톡에 전송되어 이전 미리보기가 남아있는 경우, [카카오 개발자 도구 - OG 캐시 초기화](https://developers.kakao.com/tool/clear/og)에서 해당 도메인 URL을 입력하고 **캐시 삭제**를 누르면 즉시 새로운 썸네일과 설명문으로 갱신됩니다.

---

### 2026-08-20 AI 웹사이트 빌더 독립 OpenGraph / SNS 카드 메타데이터 동적 생성 엔진 완성 (v1.53) 🟢

- **독립 OpenGraph/Twitter 카드 탑재**: `dynamic-renderer/[brand_id]` 및 모든 커스텀 클라이언트 레이아웃에 고유 `og:title`, `og:description`, `og:image`, `twitter:card` 메타데이터 파이프라인 완비.
- **다단계 자동 추출 엔진**: 첫 섹션의 실제 미디어 에셋과 본문 문단을 0ms로 분석하여 카카오톡 미리보기 카드 100% 독립 렌더링 실현.
- **서브페이지 동적 브랜딩**: `/about`, `/services`, `/blog` 등 서브페이지 공유 시에도 해당 페이지 전용 타이틀과 이미지를 반영.
