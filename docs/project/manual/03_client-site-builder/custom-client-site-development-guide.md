...
## 9. `client_sites` 마스터 DB 자동 연동 & [내 웹사이트 관리] 동기화 규칙 (Admin Integration)

커스텀 사이트(`clients/[client_id]`)는 정적 풀코드 구조이지만, 고객이 스튜디오의 **[내 웹사이트 관리] (`/studio/client-site-builder/settings`)** 메뉴에서 회사명, 전화번호, 주소, SNS 링크, GA4 ID 등을 직접 수정하고 관리할 수 있도록 자동 매핑 및 양방향 동기화가 지원됩니다:

1. **자동 매핑 (Auto-bridge)**:
   - 고객이 로그인하면 `context.tsx`가 `profiles.brand_id`를 감지하여, `client_sites` 테이블에 레코드가 없을 경우 자동으로 기본 정보를 매핑하여 `client_sites` 레코드를 생성/연결합니다.
2. **양방향 설정 동기화**:
   - 고객이 [내 웹사이트 관리]에서 정보를 수정하고 [저장]을 누르면 `client_sites` 업데이트와 동시에 `/api/clients/config` API를 호출하여 `profiles.extra_configs` 및 커스텀 사이트 런타임 캐시가 실시간 갱신됩니다.
3. **블로그 및 문의 내역 실시간 연동**:
   - `writing_creaibox_posts`에 발행된 글은 `clients/[client_id]/blog`에 즉시 노출되며, 접수된 견적문의는 고객 대시보드에서 실시간으로 확인됩니다.

---

## 10. 견적/상담 문의 폼(ContactForm) DB 연동 규약 및 고객 문의 관리 (Inquiry Management)

모든 커스텀 기업 홈페이지의 견적/상담 문의 폼(`ContactForm.tsx`)은 방문자의 문의를 Supabase `site_posts` DB에 정석 보관하고 스튜디오 관리자 화면으로 실시간 중계합니다:

### 10.1 API 엔드포인트 및 데이터 전송 스펙
- **엔드포인트**: `POST /api/client-site-builder/inquiry`
- **요청 Body**:
  ```json
  {
    "brandId": "sotongcheum",
    "formData": {
      ...
      "type": "공공행사 및 축제",
      "event_date": "2026-08-20 ~ 2026-08-22",
      "startDate": "2026-08-20",
      "endDate": "2026-08-22",
      "budget": "300만원~500만원 이하",
      "message": "세부 요구사항 기재 내용",
      "agree": true
    }
  }
  ```

### 10.2 행사 희망일 [시작일 ~ 종료일] 듀얼 피커 포맷 규칙
- **당일 행사**: 시작일만 선택 시 ➔ `YYYY-MM-DD (당일)`
- **기간 행사**: 종료일까지 선택 시 ➔ `YYYY-MM-DD ~ YYYY-MM-DD`
- **일정 미정**: 미선택 시 ➔ `일정 미정 (상담 시 협의)`

### 10.3 정형 예산 범위 셀렉트 옵션 표준
고객 이탈을 방지하고 정확한 데이터 통계를 위해 자유 텍스트 대신 표준 카테고리 드롭다운을 사용합니다:
...
### 10.4 스튜디오 [고객 문의 관리] 탭 연동
- **경로**: `/studio/client-site-builder/inquiries`
- **화면 구조**:
  - **풀-와이드 반응형 (`max-w-[1720px]`)**: 대형 모니터 및 와이드 스크린에서 100% 자동 확장.
  - **좌우 50:50 그리드 분할**: 좌측 목록 테이블(핵심 9개 컬럼 전체 노출)과 우측 상세 패널(1줄 가로 배치 컴팩트 뷰)의 완벽한 밸런스.
  - **고대비 처리 상태 변경**: `접수(블루)` / `상담중(앰버)` / `완료(에메랄드)` 원클릭 저장 및 관리자 상담 메모 지원.
