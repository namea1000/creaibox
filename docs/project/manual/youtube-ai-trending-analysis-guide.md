    ...
    rect rgb(20, 20, 30)
        ...
        alt Memory Cache Hit (최고속 10ms)
            ...
            alt DB Cache Hit (0.1초 내외)
                ...
                API->>API: YouTube API 호출 (채널 통계 수집)
                API->>API: 썸네일 이미지 다운로드 및 Base64 인코딩
                API->>AI: Gemini Pro API 호출 (텍스트 프롬프트 + 썸네일 멀티모달 전송)
...
## 3. Gemini AI 생성 및 3대 고도화 가이드라인

최초 수집(Cache Miss) 시 백엔드는 크리에이박스 관리자 API 금고(`admin_api_vault`)에서 복호화해 낸 시스템 전용 공용 Gemini API 키를 로테이션하여 사용하며, 유튜브 API 추가 호출 및 이미지 Base64 인메모리 인코딩을 거쳐 구글 Gemini Pro 멀티모달 API에 일괄 전송합니다.

### 3.1 AI 프롬프트 분석 3대 고도화 영역
Gemini 모델(`gemini-3.1-flash-lite`)에 주입되는 전문 분석 프롬프트는 멀티모달 이미지 데이터와 외부 통계 데이터를 결합하여 아래 3대 핵심 요소를 마크다운 소제목 서식으로 도출하도록 지시합니다:

1. **시청자를 매료한 핵심 바이럴 요인 (Viral Code)**:
   * **썸네일 멀티모달 시각 분석 도입**: 전송된 비디오 썸네일 이미지를 Gemini의 비전 능력을 활용해 직접 판독합니다. 이미지 내 구도, 배색 대비, 인물 표정/시선 처리, 강조 자막의 크기 및 카피 가독성이 클릭율(CTR)에 미치는 시각적 후킹 기법을 분석합니다.
2. **타겟 키워드 및 태그의 알고리즘 유효성 (Keyword & Tag Engine)**:
   * **구독자 대비 조회수 배수 (Outlier Ratio) 분석**: 유튜브 `channels.list` API로 구한 실시간 채널 구독자 수와 영상 조회수를 매핑하여 `Outlier 배수 = 조회수 / 구독자 수`를 연산합니다. 이 배수를 토대로 단순 기존 팬층 기반 노출인지, 혹은 순수 기획/썸네일 파워만으로 홈피드 추천 알고리즘을 터뜨린 '순수 기획 흥행작'인지 정밀 진단합니다.
3. **내 채널을 위한 크리에이박스 변형 기획안 (Remix Blueprint)**:
   * **영상 스크립트 플롯 및 벤치마킹 시나리오**: 일반 1인 크리에이터가 이 흥행 기획을 자사 테마에 맞춰 안전하게 리믹스할 수 있도록 기승전결 스토리 아크와 구체적인 오프닝 후킹 대사 스크립트 시나리오 템플릿을 구체적으로 제공합니다.
...
## 6. 로드맵 및 개인화 격리 정책 (Roadmap & Privacy Isolation)

서비스 스케일 및 사용자 프라이버시 정책 변화에 대응하기 위해 수립된 단계별 운영 로드맵입니다.

* **1단계: 글로벌 공용 리포트 피드 (현재)**
  * **목적**: 초기 서비스 트래픽 상황에서의 API 비용(Gemini 토큰) 극대화 절감 및 크리에이터 간 트렌드 분석 영감 상호 공유.
  * **구조**: 어드민 금고 공용 Key 및 글로벌 DB 캐싱 공유 리포트 방식.
* **2단계: 사용자별 개인화 격리 (향후 트래픽 증가 시)**
  * **목적**: 사용자가 직접 자신의 개인 API Key(YouTube / Gemini)를 사용할 경우, 기밀한 경쟁사 분석 보안 유지를 위한 프라이버시 보장.
  * **구조**: 
    * `youtube_video_analysis` 테이블에 `user_id` 외래키 컬럼 추가.
    * 조회 및 저장 API(`/api/youtube/reports`) 호출 시 현재 로그인한 사용자의 `user_id`를 매핑하여 캐싱 결과를 물리적으로 완벽 격리.
    * 자신이 분석한 영상 리포트만 본인의 보관함에 비공개 노출되도록 필터링 처리.
