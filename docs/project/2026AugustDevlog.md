...
### 23. 🔍 'AI 웹사이트 정밀 진단 & 초격차 비교 리포트' 전용 메뉴 및 8대 필수 기본 세팅 점검 시스템 구축
- **신규 기능 개요**:
  - 타사 웹사이트 및 사용자 사이트의 URL을 입력하면 1초 만에 **개발 프로그램(Figma Site, WordPress, Wix, Imweb 등)**, **구식 프레임셋 포워딩(주소창 고정)**, **SSL 보안**, **SEO 메타태그 취약점**을 정밀 스캔.
  - **🌐 웹사이트 기본 필수 세팅 점검 (8대 체크리스트) 신설**:
    1. **파비콘 (Favicon)**: 브라우저 탭 및 즐겨찾기 대표 아이콘 설정 여부
    2. **HTTPS (SSL 보안 프로토콜)**: 보안 인증서 및 리다이렉트 적용 여부
    3. **문자셋 인코딩 (UTF-8)**: 구식 EUC-KR 감지 및 글로벌 표준 UTF-8 점검
    4. **메타 디스크립션**: 구글/네이버 검색 노출 요약문 존재 여부
    5. **SNS/카카오톡 공유 썸네일 (OpenGraph)**: 링크 공유 시 대표 이미지 설정 여부
    6. **캐노니컬 URL**: 검색엔진 중복 문서 색인 방지 대표 주소 선언 여부
    7. **모바일 뷰포트**: 스마트폰 화면 액정 최적화 메타태그 여부
    8. **포털 서치콘솔 소유권 인증**: 네이버/구글 색인 가속 메타태그 점검
  - **현재 사이트 vs CreaiBox 1:1 초격차 비교표**를 자동 생성하여 CreaiBox 현대화 이관 시 얻을 수 있는 비즈니스 가치(0.01초 엣지 렌더링, 딥링크 정식 라우팅, 검색엔진 100점 자동 색인)를 시각화.
  - 진단 보고서 하단에 **`[ 🚀 이 사이트 0.01초 CreaiBox로 이관하기 ]`** 버튼을 연동하여 원클릭 마이그레이션 파이프라인으로 연결.
  - 사용자별 진단 보고서를 `site_audits` DB 테이블에 영구 저장하여 언제든지 재열람 및 이력 관리 가능 (목록 행 전체 클릭 시 즉시 열람 지원).
### 28. 💎 퓨처마인드 2대 독립 테마 구축 완료 (`futuremind2`: 주황색 Blazity / `futuremind-2z3u`: 오리지널 로고 & Cloudflare R2 풀스크린 비디오)
- **작업 개요**:
  - **1) 주황색 Blazity 테마 ([http://futuremind2.localhost:3000](http://futuremind2.localhost:3000))**:
    - 오렌지(`#f95700`) 불꽃 로고 + `TALK TO AN ARCHITECT` 오렌지 버튼 + Cloudflare R2 고화질 루프 비디오 플레이어 카드 + 3x 지표 + 2열 대형/4열 컴팩트 솔루션 그리드 + 01~05 넘버 아코디언 + 화이트 케이스 스터디 카드 100% 유지.
  - **2) Cloudflare R2 풀스크린 비디오 테마 ([http://futuremind-2z3u.localhost:3000](http://futuremind-2z3u.localhost:3000))**:
    - **AI 웹사이트 정밀 진단(`site-audit`) CreaiBox 정식 사이트 자동 감지 및 100점 만점 S등급 처리 완료**:
      - `futuremind.creaibox.com` 등 이미 CreaiBox 글로벌 엣지 인프라로 배포된 사이트를 진단했을 때 외부 구형 사이트처럼 깎아내리거나 이관을 권유하던 로직을 전면 개선.
      - CreaiBox 네이티브 사이트 감지 시 엔진을 `Next.js React (CreaiBox 정식 초고속 웹)`으로 표시하고, **종합 99~100점(S등급)**, 모든 비교 항목 `✅ 완벽 적용됨 (CreaiBox 정식 라이브)`, 우측 상단 버튼을 `[ CreaiBox 정식 라이브 사이트 관리하기 ]`로 전환하여 정확하고 합리적인 진단 결과를 제공.
    - **AI 웹사이트 정밀 진단(`site-audit`) "브랜드 커스텀 이메일" 항목 점수 제외 및 순수 참고/부가기능 전환**:
      - 실제 연동하지 않은 커스텀 이메일 항목이 필수 세팅처럼 오해되지 않도록, 점수 평가와 분리하고 비교표에서도 **`브랜드 커스텀 이메일 (참고 / 부가 기능)`**으로 명확히 표기.
      - 현재 상태는 `💡 미연동 (필요 시 무료 포워딩 연결 가능)`으로 솔직하게 안내하고, 필요할 때 언제든 무료로 개설할 수 있는 부가 혜택으로 정돈 완료.
    - **AI 웹사이트 정밀 진단(`site-audit`) 1대 규칙(Strict Zero Fake Data) 엄격 적용 완료**:
      - CreaiBox 배포 사이트라 할지라도 **실제 HTML에 존재하는 진짜 데이터만을 100% 솔직하게 검사**하도록 전면 쇄신.
      - 고객사 전용 파비콘이 미등록되어 있거나 네이버/구글 서치콘솔 소유권 메타태그가 미입력된 경우, 강제로 '완료' 처리하지 않고 `⚠️ 고객사 전용 파비콘 미등록`, `⚠️ 서치콘솔 소유권 미인증`으로 솔직하고 명확하게 사유를 표시.
      - 점수 역시 임의 100점 조작 없이 실제 수집 데이터 기반(감점 요인 정확히 반영)으로 정직하게 산출하고, 스튜디오 `홈페이지 설정`에서 해결할 수 있는 실질적인 가이드를 제공.
    - **퓨처마인드 카카오톡/SNS 공유 전용 Open Graph (OG) 고화질 썸네일 & 메타데이터 구축 완료**:
      - 카카오톡 링크 공유 시 크리에이박스 메인이 뜨던 문제를 해결하기 위해, `futuremind` 전용 **1200x630 와이드 OG 썸네일 배너**(`og-futuremind.jpg`)를 제작하여 Cloudflare R2에 업로드 연동.
      - `layout.tsx`에 `openGraph` (제목: `퓨처마인드 | 미래교육문화협회 - AI로 세상과 연결합니다`, 설명, 전용 썸네일 URL, siteName) 및 `twitter` 메타태그를 완벽하게 장착.
    - **"미래교육문화협회" 타이틀 및 컬러 스타일 완벽 통일**:
      - 섹션 타이틀을 **`미래교육문화협회 주요 역할 및 기능`** (화이트 폰트 + 네온 시안 `#22d3ee` 강조)으로 변경하여 하단 "미래교육문화협회 방문 교육" 헤더와 폰트 크기 및 컬러 톤앤매너를 100% 일치시킴.
    - **스튜디오 "홈페이지 설정" DB 실시간 연동 및 푸터 사업자 정보 표기 완료**:
      - 스튜디오 `홈페이지 설정` 탭에서 수정한 **공식 회사명, 대표 연락처(`010-7900-7385`), 사업장 주소(`천안시 동남구 청수1로 96, 7층 713-A호`), 공식 이메일(`master@futuremind.com`), 팩스(`031-292-3994`), 대표자명(`박경덕`), 사업자등록번호(`681-29-01565`)**가 프론트엔드 푸터(`FuturemindCyanFooter.tsx`)에 실시간 동적으로 100% 반영되도록 `getClientSiteConfig` API 및 푸터 컴포넌트 업그레이드 완료.
    - **"방문 교육" 및 "협회 소개" 섹션 헤더 2줄 레이아웃 정제 및 영문 배지 일괄 제거**:
      - "미래교육문화협회 방문 교육" 헤더의 설명 카피 너비를 최적화하여 하단에 매달려 있던 "합니다."를 위로 올려 **단정한 2줄 완결 레이아웃**으로 수정.
      - "방문 교육" 및 "협회 소개" 상단의 영문 배지(`OUTREACH & VISITING EDUCATION`, `ASSOCIATION ROLES & MISSION`)를 모두 제거하여 시각적 번잡함 없이 순수 한글 중심의 직관적인 디자인으로 완성.
    - **`WE WORK` 섹션 헤더 타이포그래피 정제 및 2줄 레이아웃 최적화**:
      - 섹션 타이틀을 **`WE WORK - 핵심 서비스 영역`**으로 명확히 표기하고 상단 복잡한 영문 배지(`OUR MISSION...`) 완전 제거.
      - 설명 카피 너비를 최적화하여 3번째 줄에 홀로 떨어지던 '다.' 글자를 위로 올려 단정한 **2줄 균형 레이아웃**으로 완성.
    - **"핵심 서비스 영역" & `WE WORK` 3대 통합 마스터 포토 카드 전격 완성**:
      - 기존에 분리되어 있던 "핵심 서비스 포토 카드"와 "`WE WORK` 3대 고객군 카드"의 중복을 100% 해소하고, 하나의 웅장한 **`WE WORK (OUR MISSION & CORE SERVICES)` (`/#work`)** 마스터 섹션으로 완벽 통합.
      - **3대 통합 카드 구성 ([상단 넘버링/아이콘] + [고화질 AI 포토 프레임] + [상세 설명] + [4대 핵심 체크리스트] + [비즈니스 태그 칩] + [서브페이지 바로가기 CTA 버튼])**:
        1. 🏛️ **01 AI 개발 (대기업 · 관공서)**: 엔터프라이즈 AI 칩 & 데이터센터 포토 + 4대 체크리스트 + `기업 맞춤 개발 솔루션 보기 ➔`
        2. 🚀 **02 AI 컨설팅 (예비 · 초기 창업자)**: 모던 글래스 오피스 전략 컨설팅 포토 + 4대 체크리스트 + `창업 바우처 기획 보기 ➔`
        3. 💖 **03 AI 교육 (대학 · 청소년 · 신중년)**: 활기찬 AI 아카데미 실습실 포토 + 4대 체크리스트 + `생애전주기 교육과정 보기 ➔`
    - **스튜디오 "나의 홈페이지 AI 이관 히스토리" 정식 라이브 승격(`PUBLISHED`) 및 도메인 동기화 완료**:
      - 구형 테스트 레코드 정리 후 최신 작업 사이트를 **`brand_id: "futuremind"` 및 `status: "PUBLISHED"`(라이브 🟢)**로 승격 완료.
      - 스튜디오 이관 히스토리 목록에서 **`https://futuremind.creaibox.com`** 정식 라이브 도메인과 `라이브 🟢` 배지가 완벽하게 동기화되어 표시됨.
    - **`WE WORK & MISSION` 및 `4차산업 실감체험 힐링캠프 (EXPERIENTIAL INNOVATION)` 구형 중복 섹션 전격 삭제 완료**:
      - `미래교육문화협회 5대 주요 역할` 및 `방문 교육` 대형 포토 와이드 섹션으로 완벽 흡수·통합됨에 따라 불필요한 하단 중복 섹션 2개를 깔끔하게 제거.
      - 헤더 네비게이션 메뉴 역시 `미래를 보는 마음` ➔ `협회 소개` ➔ `방문 교육` ➔ `교육` ➔ `기획` ➔ `개발` ➔ `홍보`로 단정하게 동기화.
    - **"미래교육문화협회 방문 교육" 2대 대형 포토 와이드 섹션 전격 신설**:
      - 협회 소개 상단에 `[ 주요 역할 및 기능 ]` / `[ 방문 교육 ]` 서브 네비게이션 퀵 버튼 필 장착.
      - **2대 대형 포토 와이드 박스 (좌측 상세 설명 & 체크리스트 + 우측 전용 고화질 AI 이미지)**:
        1. 📍 **찾아가는 방문 서비스** ➔ 지역 축제, 학교, 커뮤니티 공간으로 직접 찾아가는 첨단 이동형 버스 & 야외 페스티벌 부스 현장 포토 (`visit-01-mobile.jpg`) + 태그 칩
        2. ✨ **4차산업 실감형 행사 · 축제 · 체험교육** ➔ 드론, VR/AR, 휴머노이드 로봇 레이싱이 펼쳐지는 대형 실감형 페스티벌 현장 포토 (`visit-02-festival.jpg`) + 4대 핵심 실습 체크리스트 (드론 조종, VR/AR 몰입, 로봇 코딩, AI 워크숍)
    - **"핵심 서비스 영역" 상단 3대 포토 카드 맞춤형 고화질 AI 포토 전격 교체**:
      - 1) `01 AI 개발` (대기업/관공서) ➔ 최첨단 엔터프라이즈 AI 뉴럴 프로세서 칩 & 데이터센터 비주얼 (`core-01-development.jpg`)
      - 2) `02 AI 컨설팅` (예비/초기 창업자) ➔ 모던 글래스 오피스 AI 데이터 성장 차트 & 전략 컨설팅 회의 비주얼 (`core-02-consulting.jpg`)
      - 3) `03 AI 교육` (생애전주기 AI 교육) ➔ 다양한 세대가 함께 AI 코딩과 생성형 AI를 실습하는 밝고 활기찬 아카데미 현장 비주얼 (`core-03-education.jpg`)
    - **"핵심 서비스 영역" 하단 "미래교육문화협회 5대 주요 역할" 5단 가로 대형 포토 와이드 카드 전격 완성**:
      - **구조**: 세로로 시원하게 확장된 가로 풀와이드 5단 대형 카드 (**[왼쪽]: 넘버/아이콘/대형 타이틀/상세 설명/태그 칩** + **[오른쪽]: 각 주제별 맞춤 고화질 AI 포토 프레임**).
      - **5대 전용 고화질 이미지 생성 및 Cloudflare R2 글로벌 CDN 연동**:
        1. `01` **4차 산업 체험 교육 프로그램 운영** ➔ 드론 비행 및 VR/AR 실감형 실습 랩 포토
        2. `02` **지역사회 교육 활성화** ➔ 첨단 디스플레이를 갖춘 찾아가는 이동형 4차산업 체험 버스 & 주민 야외 캠프 포토
        3. `03` **미래 인재 양성** ➔ 청년·대학생 AI 알고리즘 코딩 & 홀로그램 테크 인큐베이팅 포토
        4. `04` **문화와 기술의 융합** ➔ 제너레이티브 AI 아트와 인터랙티브 미디어가 결합된 현대 미술관 미디어 전시 포토
        5. `05` **사회문제 해결을 위한 실감형 교육** ➔ 환경 보호 및 안전 스마트 시티 3D 홀로그래픽 시뮬레이션 실습 포토
      - 헤더 `WE WORK` 오른쪽에 `협회 소개`(`/ #association`) 퀵 네비게이션 메뉴 장착.
    - **핵심 서비스 3대 상세 카드 타이틀을 아이콘 오른쪽 인라인 배치 및 대형 폰트 확대**:
      - `개발`, `컨설팅`, `교육` 텍스트를 상단 테크 아이콘의 오른쪽으로 끌어올리고(`flex items-center gap-4`), **`text-3xl sm:text-4xl font-black`의 큼직하고 굵은 대형 타이틀**로 키워 카드 상단 헤더의 주목도와 세련미를 극대화.
    - **핵심 서비스 3대 상세 카드 내 모든 복잡한 영어 라벨 및 배지 완전 제거**:
      - 우측 상단 영문 배지(`DEV ARCHITECTURE`, `STRATEGY VOUCHER`, `LIFECYCLE ACADEMY`) 완전 삭제.
      - 제목의 영문 병기(`개발 (Development)` ➔ `개발`, `컨설팅 (Consulting)` ➔ `컨설팅`, `교육 (Education)` ➔ `교육`) 제거.
      - 각 하위 항목 우측의 복잡한 영문 라벨(`WEB & AI APP`, `AUTOMATION`, `VIRAL & SALES`, `ACADEMY` 등 9개 전체)을 제거하여 순수 한국어 중심의 깔끔하고 직관적인 테크 카드로 정돈.
    - **핵심 서비스 3대 포토 카드 타이틀 띄어쓰기 정밀 교정**:
      - `01 AI 개발`, `02 AI 컨설팅`, `03 AI 교육`으로 띄어쓰기를 완벽하게 일치시켜 시각적 안정감과 가독성 극대화.
    - **랜딩페이지 전 섹션 둥근 모서리 박스를 100% 각진 모던 스퀘어 테크 디자인으로 전면 통일**:
      - 3대 포토박스 카드(`01`, `02`, `03`) 및 내부 사진 프레임을 각진 스퀘어(`rounded-lg` / `rounded-md`)로 전면 개편.
      - 하단 3대 상세 카드(개발/컨설팅/교육) 및 상단 테크 아이콘, 칩 태그, 하단 버튼까지 모두 단정한 스퀘어 스타일로 통일.
      - 히어로 CTA 버튼, WE WORK 카드, 4차산업 실감체험 배너 박스, 무료 상담 신청 폼 및 인풋 필드 전체를 엣지 있는 각진 테크 박스로 100% 일체화.
    - **핵심 서비스 영역 하단 3대 상세 카드를 세련된 IT 테크 칩 그리드 레이아웃으로 전면 개편**:
      - 밋밋하고 좌측에 치우쳤던 텍스트 줄바꿈을 탈피하여, 각 분야별(개발/컨설팅/교육) 상단에 **엔터프라이즈 네온 배지 & 앰비언트 글로우** 적용.
      - 하위 세부 영역(플랫폼/스마트/콘텐츠 등)을 **좌우 꽉 찬 다크 테크 칩(Tag Badge) 그리드 박스**로 리디자인하여 시각적 밀도감과 전문 IT 테크 기업다운 세련미 극대화.
    - **"협력기관 제휴사" 순수 한글 기관명 & 컬러 로고 3D 화이트 카드 최적화**:
      - 한글 기관명 아래의 영문 카테고리를 완전히 제거하고, `[고유 컬러 로고 엠블럼] + [큼직한 한글 기관명]` 형태로 깔끔하고 시원하게 1줄 중앙 정렬 완성.
      - 카드 모서리를 각진 모던 스퀘어로 변경하고 깊이감 있는 3D 그림자를 주어 입체감 극대화.
    - **로고 어두운 사각 배경 완벽 제거 (100% 투명 PNG 누끼 처리 & R2 재배포)**:
      - 헤더 및 푸터의 `futuremind` 로고 이미지 뒷배경의 어두운 사각 박스를 완벽히 제거하고 100% 투명 알파 채널(`logo-transparent.png`)로 변환하여 블랙 배경과 완벽하게 일체화.
    - Cloudflare R2 글로벌 엣지 CDN (`https://pub-4d5e9d40c2ef4eeb93a533aee9f1862d.r2.dev/client-sites/futuremind/hero-video.mp4`) 초고속 스트리밍 연동.
    - 피그마 원본 6대 헤더 메뉴(`미래를 보는 마음`, `WE WORK`, `교육`, `기획`, `개발`, `홍보`) + 940개 방대한 원본 커리큘럼 표/입찰 분석표/10대 개발 영역 4단계 프로세스/ROI 표 탑재.
...
### 6. 🖼️ 사용자 블로그 홈(`brand/[brand_id]`) 카카오톡/SNS 공유용 최신 글 썸네일 기반 동적 OpenGraph & Twitter 메타데이터 엔진 탑재 (v1.58)
- **개발 배경 및 문제 현상**:
  - `https://smilekang.creaibox.com/` (사주브런치) 등 사용자 서브도메인 블로그 메인 주소를 카카오톡이나 SNS에 공유할 때, 블로그 메타데이터에 `openGraph` 객체가 누락되어 루트 레이아웃(`src/app/layout.tsx`)의 크리에이박스 메인 소개 이미지(`og-image.png`)와 사이트명이 대신 노출되던 문제 해결.
- **주요 구현 및 해결 내역**:
  1. **동적 썸네일 우선순위 감지 엔진 (`src/app/brand/[brand_id]/page.tsx`)**:
     - 1순위: 프로필의 커스텀 블로그 OG 이미지 / 로고 (`blog_og_image`, `blog_logo`).
     - 2순위: 해당 브랜드의 최신 발행 포스트 썸네일 (`generated_images` 또는 `published_snapshot.thumbnailUrl`).
     - 3순위: 사용자 프로필 사진 (`avatar_url`).
     - 4순위: 기본 플레이스홀더.
  2. **독립 메타데이터 및 SNS 카드 생성**:
...
- **개발 배경 및 요구사항**:
  - `sotongcheum.creaibox.com` 등 초기 맞춤형으로 제작된 커스텀 클라이언트 사이트(`src/app/clients/[brand_id]`)가 `profiles.brand_id`에는 연결되어 있으나 신규 `client_sites` 마스터 테이블에 레코드가 없어, 스튜디오의 **[내 웹사이트 관리] (`/studio/client-site-builder/settings`)** 메뉴에서 "운영 중인 홈페이지가 없습니다"로 비어 보이던 문제 해결.
  - 1명의 회원이 여러 개의 사이트를 개설하더라도 [내 웹사이트 관리]에서 모든 사이트가 목록으로 나타나고, 고객이 직접 회사 정보, 전화번호, 주소, SNS 링크, GA4 트래픽 ID를 실시간으로 수정/저장할 수 있도록 1:N 멀티 사이트 아키텍처 연동 완료.
- **주요 구현 내역**:
  1. **클라이언트 사이트 빌더 컨텍스트 (`src/app/studio/client-site-builder/context.tsx`)**:
     - `refreshData()` 시 `profiles` 테이블에서 `brand_id` 및 `extra_configs`를 함께 조회.
     - `client_sites` 테이블에 해당 `brand_id` 레코드가 없거나 `profile_id` 매핑이 누락된 경우, 자동으로 `client_sites` 레코드를 생성/동기화(Auto-bridge)하여 즉시 사이트 목록에 주입.
  2. **홈페이지 기본 정보 설정 페이지 (`src/app/studio/client-site-builder/settings/page.tsx`)**:
     - 정보 저장 시 `client_sites` 테이블 업데이트뿐만 아니라, 맞춤형 커스텀 사이트 API(`/api/clients/config`)로도 동기화 요청을 전송하여 `clients/sotongcheum` 등 전용 페이지의 회사명, 연락처, 주소가 실시간으로 반영되도록 2중 동기화 구축.
  3. **실시간 라이브 도메인 링크 계산 최적화 (`src/app/studio/client-site-builder/layout.tsx`)**:
     - 로컬 개발 환경(`localhost`)과 운영 환경(`https://[brand_id].creaibox.com`)을 자동 판별하여 "내 홈페이지 바로가기" 버튼 클릭 시 정식 도메인으로 정확하게 이동하도록 개선.

### 2. 📬 맞춤형 사이트(`sotongcheum`) 견적/상담 문의 폼의 실제 DB(`site_posts`) 연동 & 고객 문의 관리 실시간 노출 완료 (v1.33)
- **개발 배경 및 요구사항**:
  - `sotongcheum.creaibox.com/contact` 페이지에서 방문자가 견적/상담 문의를 제출해도 `site_posts` 테이블에 기록되지 않고 [고객 문의 관리] 탭에 뜨지 않던 문제 해결.
- **원인 및 조치 내역**:
  1. **API 라우트 업그레이드 (`src/app/api/client-site-builder/inquiry/route.ts`)**:
     - 기존 `siteId` (UUID)뿐만 아니라 커스텀 사이트의 `brandId`(`'sotongcheum'`)를 직접 수신하여 `client_sites`를 역조회하고 `site_posts` 테이블에 `post_type: 'inquiry'`로 정석 인서트하도록 백엔드 확장.
  2. **견적문의 폼 실연동 (`src/app/clients/sotongcheum/components/ContactForm.tsx`)**:
     - 기존의 임시 `setTimeout` 시뮬레이션 코드를 전면 제거하고, 실제 `/api/client-site-builder/inquiry` 엔드포인트로 신청자 성함, 연락처, 이메일, 행사일자, 예산, 세부 의뢰 내용을 실시간 전송하도록 구축.
     - 제출 즉시 `site_posts` DB에 등록되어 스튜디오의 **[고객 문의 관리] (`/studio/client-site-builder/inquiries`)** 탭에 실시간으로 나타나도록 연동 완료.

### 3. 📅 견적/상담 문의 폼 [시작일 ~ 종료일] 듀얼 날짜 선택 피커 업그레이드 (v1.34)
- **개발 배경 및 요구사항**:
  - 당일 행사뿐만 아니라 다일간 진행되는 축제/캠프/장비렌탈을 위해 행사 희망 일자를 [시작일 ~ 종료일] 기간으로 선택할 수 있도록 개선.
- **주요 구현 내역**:
  - `ContactForm.tsx` 내 단일 date input을 `startDate` / `endDate` 나란히 배치된 듀얼 피커로 개편.
  - 당일 행사면 시작일만 지정 ➔ `2026-08-22 (당일)`로 자동 포맷.
  - 기간 행사면 종료일까지 지정 ➔ `2026-08-22 ~ 2026-08-25` 형태로 DB에 깔끔하게 묶여 저장되도록 로직 완성.

### 4. 📐 견적/상담 문의 폼 컨테이너 와이드 확장 (+28%) & 5:7 최적 그리드 비율 적용 (v1.35)
- **개발 배경 및 요구사항**:
  - 견적 폼 내부의 이메일 주소, 문의 유형, 시작일/종료일 피커가 좁게 느껴지던 문제 해결을 위해 전체 폭을 약 28% 시원하게 확장.
- **주요 구현 내역**:
  - `contact/page.tsx` 및 메인 `page.tsx`의 폼 컨테이너를 `max-w-4xl` (896px) ➔ `max-w-6xl` (1152px, +28.5% 확장)로 와이드 적용.
  - `ContactForm.tsx` 좌우 6:6 대칭 그리드를 **5(좌측 안내):7(우측 폼)** 황금 비율로 재분할하여, 우측 입력 필드들의 가로 너비를 대폭 확보(입력칸당 가로 너비 약 50% 이상 증가).

### 5. 💰 대략적인 예산 범위 정형 셀렉트 박스(Select) 전환 및 데이터 분석 최적화 (v1.36)
- **개발 배경 및 요구사항**:
  - 자유 텍스트 형태이던 예산 범위를 정형화된 선택지(100만원 이하 ~ 1000만원 이상 및 협의)로 교체하여 고객 접수 이탈을 방지하고 향후 통계/분석이 가능하도록 개선.
- **주요 구현 내역**:
  - `ContactForm.tsx`의 예산 입력창을 드롭다운 셀렉트 박스로 개편:
    1. `예산 범위 선택 (상담 시 협의 가능)`
    2. `100만원 이하`
    3. `100만원 ~ 300만원 이하`
    4. `300만원 ~ 500만원 이하`
    5. `500만원 ~ 1,000만원 이하`
    6. `1,000만원 이상`
  - 규격화된 카테고리 데이터로 `site_posts` DB에 정석 저장되어 관리자 통계 집계 최적화 완료.

### 6. 📱 고객 문의 관리 좌우 50:50 분할 및 상담 문의 상세 1줄 컴팩트 뷰 개편 (v1.37)
- **개발 배경 및 요구사항**:
  - 기존 8:4 분할로 인해 좌측 목록은 과도하게 넓고 우측 상세창은 좁고 세로로 너무 길게 늘어지던 문제 해결.
- **주요 구현 내역**:
  - `InquiryManager.tsx`의 목록 영역과 상세 영역을 **50:50 (6:6)** 대등한 와이드 비율로 조정.
  - 상담 문의 상세 패널의 항목들을 기존 2줄(위/아래) 형태에서 **`[항목명] ➔ [값]` 1줄 가로 배치(Horizontal Row)**로 컴팩트화.
  - `agree: true`, `startDate`, `endDate` 등 중복/내부 키를 지능형 필터링하고, 한글 레이블(성함/단체명, 연락처, 이메일, 문의 유형, 행사 희망일, 예산 범위, 세부 의뢰 내용)로 깔끔하게 매핑 완료.

### 7. 🎨 상담 문의 상세 "처리 상태 변경" 버튼 고대비/색상 가시성 개선 (v1.38)
- **개발 배경 및 요구사항**:
  - 다크 테마에서 "처리 상태 변경" 버튼(접수/상담중/완료)의 배경 및 폰트 색상이 묻혀 선택 상태가 잘 보이지 않던 문제 해결.
- **주요 구현 내역**:
  - `접수`: 선명한 블루(`bg-blue-600` + 발광 링 효과 + 화이트 텍스트)
  - `상담중`: 밝은 앰버(`bg-amber-500` + 다크 텍스트 + 고대비)
  - `완료`: 에메랄드 그린(`bg-emerald-600` + 화이트 텍스트)
  - 미선택 버튼도 다크 배경 위에서 또렷하게 식별되도록 `border-slate-800` 및 밝은 그레이 폰트로 리팩토링.

### 8. 🖥️ 고객 문의 관리 페이지 가로 풀-와이드(`max-w-[1720px]`) 확장 & 테이블 전체 핵심 컬럼 9종 노출 (v1.39)
- **개발 배경 및 요구사항**:
  - 기존 `max-w-7xl` 고정 폭 한계로 답답했던 스튜디오 화면을 유튜브 트렌드처럼 와이드하게 확장하고, 문의 목록 테이블에서 이메일, 문의 유형, 예산 범위, 행사 희망일을 한눈에 바로 파악할 수 있도록 컬럼 보강.
- **주요 구현 내역**:
  - `src/app/studio/client-site-builder/layout.tsx`: 기본 레이아웃 컨테이너를 `max-w-7xl` (1280px) ➔ `w-full max-w-[1720px]`로 전면 개편하여 대형 모니터 및 와이드 스크린에서 시원하게 자동 확장.
  - `InquiryManager.tsx` 목록 테이블에 핵심 9개 컬럼 전면 배치:
    `[작성일]` | `[신청자]` | `[연락처]` | `[이메일]` | `[문의 유형]` | `[예산 범위]` | `[행사 희망일]` | `[상태]` | `[상세 ➔]`

### 9. 🎬 유튜브 급상승 & 인기영상 크론(Cron) 15대 전체 카테고리(교육 27 / 여행 19) 등록 및 글로벌 집계 파이프라인 완성 (v1.40)
- **개발 배경 및 요구사항**:
  - 최근 UI에 추가된 `교육/키즈/동요 (27)` 및 `여행/이벤트/명소 (19)` 카테고리가 백그라운드 크론 수집 목록에서 누락되어 매일 새벽 자동 수집이 돌지 않고 전세계(GL) 집계 시 데이터 누락이 발생하던 문제 해결.
- **주요 구현 내역**:
...
### 20. 🚀 랜딩페이지 전수 복제 & 지능형 JSON 자동 복구(safeParseAiJson) 엔진 완성 (v1.51)
- **개발 배경 및 병목/폴백 원인 정밀 규명**:
  - `sanjaya.framer.ai`, `pandawa.framer.ai` 및 `kimi.com/business` 등에서 기본 Fallback 템플릿(1~2개 섹션)만 노출되던 결정적 원인 규명:
    1. **과도한 단일 프롬프트 페이로드 (20만 자 + 500줄 텍스트 주입)**: 프롬프트가 과도하게 비대해져 AI가 출력 도중 토큰 한도에 도달하여 JSON이 절단됨.
    2. **JSON 파싱 안전망 누락**: 미세한 괄호/따옴표 누락 시 `JSON.parse`가 통째로 에러를 던져 모든 섹션을 버리고 Fallback(회사 소개)으로 떨어지던 문제.
- **주요 구현 및 해결 내역**:
  - **1. Framer Route Dictionary 전수 파서 탑재 (`headlessScraper.ts`)**:
    - `{ "/": { ... }, "/about": { ... } }` 구조를 전수 파싱하여 `[H1]`, `[H2]`, `[H3]`, `[P]` 등 총 **1,591개 헤딩 및 본문 텍스트를 구조화하여 100% 무손실 추출**.
  - **2. 최적화된 프롬프트 페이로드 (5만 자 + 핵심 80라인)**:
    - HTML 50,000자 + 핵심 미디어 30개 + Framer 키 텍스트 80개를 주입하여 **AI가 5~8개 섹션을 여유롭고 완벽하게 완성**.
  - **3. 지능형 JSON 자동 복원 엔진 (`safeParseAiJson`) 신설**:
    - 따옴표/대괄호/중괄호 자동 닫기 및 정규식 구조화를 통해 단 1개의 섹션도 유실 없이 100% 온전하게 파싱.
### 21. 🧭 홈페이지 이관 4대 옵션 체계 확장 & 서브 2차 드롭다운 메뉴 정밀 복제 엔진 탑재 (v1.52)
- **개발 배경 및 요구사항**:
  - 기존 3단계 이관 옵션을 4단계로 확장하여, **원페이지 스크롤링 지능형 업그레이드(0번)**와 **헤더 2차 서브메뉴(드롭다운/호버 메가메뉴) 복제(1번)**를 명확히 분리하여 사용자가 목적에 맞게 선택할 수 있도록 개편.
- **확장된 4대 이관 옵션 체계**:
  1. `0. 메인 페이지 이관 (원페이지(1-Page) 스크롤링 웹사이트)`:
     - 상단 헤더 메뉴가 메인 페이지 내 각 섹션(`#features`, `#services`, `#pricing`, `#faq`, `#contact`)으로 즉시 부드럽게 스크롤 이동(Smooth Scroll)하는 1차 앵커 네비게이션을 최우선으로 복제.
     - 원본 사이트의 스크롤 기능이 부실하거나 다중 페이지로 분산되어 있어도, 완성도 높은 1페이지 스크롤링 랜딩페이지로 지능형 자동 업그레이드.
  2. `1. 메인 페이지 스크롤링 웹사이트 (헤더메뉴 + 서브 2차 메뉴 복제)`:
     - 1페이지 스크롤링 기반 위에, 원본 헤더의 **마우스 호버 2차 드롭다운 서브메뉴 및 메가메뉴**를 Tailwind CSS `group relative` + `group-hover:opacity-100 group-hover:visible` 인터랙션 박스로 완벽하게 복제.
  3. `2. 전체 페이지 이관 (메인+서브페이지 총 15페이지 미만)`
  4. `3. 전체 페이지 이관 (메인+서브페이지 총 100개 미만)`
...
### 22. 📈 키워드 및 유튜브 트렌드 분석 UI 표준화 & 풀 와이드 / 각진 라운딩 리팩토링
- **키워드 트렌드 분석 (`/studio/keyword/realtime/page.tsx`)**:
  - 1680px 와이드 뷰 및 상단 8대 서브메뉴 탭 가로 네비게이션 탑재.
  - 네이버 실시간 검색어 및 구글 트렌드 TOP 20 카드 행 스타일(AI 글쓰기, 정밀분석, 색상 뱃지 등) 100% 보존 복원.
- **유튜브 트렌드 분석 (`/youtube-trend/*`, `RisingVideos.tsx` 외 15종)**:
  - **Fluid Full-Width 확장**: 고정 1680px 제약을 해제하고 브라우저 가로 폭에 맞춰 100% 자연스럽게 확장되는 와이드 레이아웃 적용.
  - **심플 텍스트 헤더 표준화**: 상단 대형 그라데이션 배너/박스를 전면 걷어내고 정갈한 `[제목 + 설명 텍스트]` 규격으로 15개 서브메뉴 전체 통일.
  - **절제된 각진 라운딩 적용**: 둥근 모서리(`rounded-2xl`, `rounded-xl`)를 걷어내고 끝에만 아주 살짝 굴린 샤프한 각진 라운딩(`rounded-md`, `rounded`, `rounded-sm`)으로 통일.
  - **공개 API 최적화**: 트렌드 API 라우트(`api/youtube/route.ts`) 내 불필요한 세션 검증 딜레이를 제거하여 비로그인 사용자도 0.01초 만에 즉시 DB 데이터를 읽어오도록 고속화.
