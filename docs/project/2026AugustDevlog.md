...
### 6. 💳 이용약관 및 환불정책 내 '크레딧' 관련 문구 전면 삭제 (NHN KCP PG 심사 조치)
- **원인**: NHN KCP 심사팀(정욱환 담당자) 지시사항에 따라, 업력 2년 미만 신생 사업자의 경우 단독 '크레딧 충전' 상품을 환불 규정에 명시 시 재무제표 제출 요구 또는 승인 반려가 발생함.
- **조치 내용**:
  - `src/app/terms/page.tsx` (제5조 서비스 이용 요금 및 환불 규정): '단독 크레딧 충전 상품', '단독 크레딧 구매일', '크레딧 차감 내역' 관련 표기를 전면 삭제하고 월 구독(멤버십) 전용 조항으로 정리.
  - `src/app/refund-policy/client.tsx` & `page.tsx`: 4대 환불 가이드라인 및 메타데이터에서 '크레딧 구매' 관련 문구를 모조리 정제 및 완전 제거.
