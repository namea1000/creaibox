...
### 2단계: 썸네일 리사이징 도입 (프론트엔드 최적화)
* 미디어 라이브러리 그리드 목록에서는 고화질 원본 이미지 대신, Supabase/Google API 또는 프록시 단에서 축소 가공한 **수십 KB 수준의 가벼운 썸네일 이미지**를 우선 호출하여 Egress 대역폭 소모를 90% 이상 차단합니다.
* 사용자가 상세 보기 및 다운로드 버튼을 클릭할 때만 원본 고해상도 이미지를 내려주어 속도와 비용을 동시에 최적화합니다.
