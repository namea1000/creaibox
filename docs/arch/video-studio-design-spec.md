...
Memory, Caching, and FFmpeg Concurrency:
- Pre-Extracted Audio Caching: Loading massive raw video files (especially 4K MOV recordings) into memory to extract audio tracks causes browser crashes and `Failed to fetch` errors. The editor runs a background task using FFmpeg to extract the audio track as a lightweight WAV file upon asset import or relinking. The WAV cache is stored in IndexedDB under `audio-extract-[mediaId]`. Waveform rendering, audio mixing, and preview playback check this cache first, resulting in substantial memory savings.
- FFmpeg Concurrency Lock: FFmpeg WASM execution is single-instance and highly resource-intensive. Simultaneous invocations for reversing, scene change detection, or WebM-to-MP4 conversions can cause memory corruption. All FFmpeg commands are scheduled through a global queue lock `runWithFFmpegLock` to enforce sequential processing.
- Cache Quota Guardrail: Writing files larger than 500MB to IndexedDB often leads to browser-level sqlite thread blocking or write crashes. Source video files exceeding 500MB bypass IndexedDB storage, while their tiny pre-extracted WAV audio caches (<10MB) continue to be stored to keep waveforms and audio preview intact on reload.
...
Timeline editor interface decisions:
- Remove redundant "+ 트랙 추가" (Add Track) buttons inside the grid area to reduce UI clutter and rely on the header-level "+Track 추가" button.
- Multi-clip marquee selection: Uses mouse pointer coordinates relative to the timeline grid. Computing marquee boundaries uses client bounding rect calculations to intersect clip offsets, highlighting selected items instantly. Additive selection via modifier keys (Shift, Cmd/Ctrl) is preserved to allow manual tuning of selection sets.
- Custom bottom scrollbar: Designed to overlay the timeline grid area as a floating element rather than pushing content layout. To prevent the custom scrollbar track from blocking clicks on the lower timeline tracks, the scrollbar container has hover states where pointer events are deactivated/ignored (`pointer-events-none`) when the scrollbar is collapsed/invisible and only activated (`pointer-events-auto`) when hovered and expanded. The scrollbar itself is 20px-24px high for easy grabbing, but wraps in a larger 32px touch area.
- Boundary edge auto-scrolling: Auto-scrolling utilizes a requestAnimationFrame-driven loop triggered whenever the cursor is active near the left or right 80px boundary zones of the timeline viewport. Scrolling rates adjust linearly based on the pointer position's depth within the boundary zone to guarantee responsive and smooth panning during drags.
- Playhead paste advancement: In sequence-based assembly, pasting clips automatically updates `currentTime` to the rightmost boundary of the pasted items, allowing rapid consecutive pasting without manual playhead scrubbing.
- Background task overlays: Reversing clips and scene change detection run as background FFmpeg actions. A glassmorphic loading backdrop overlay is displayed over the editor shell to capture user clicks and display current stage messages (e.g., "장면 분석 중...") with percentages, preventing race conditions or multiple concurrent actions.
- Toolbar and inspector controls: Action buttons for "역재생" (reverse) and "장면 분할" (scene change split) are placed in both the timeline header and the right inspector panel. Reversing is enabled for both video and audio, whereas scene splitting is enabled exclusively for video clips.
- Media duration constraints: Unlike synthetic assets (image, text, subtitle, visualizer) which can be stretched indefinitely, video and audio clips are limited to their original source media duration. Right-edge dragging is capped at `media.duration - trimStart`. Left-edge dragging checks if it is a trim operation (right edge fixed); if so, `trimStart` is updated by the change in `startTime`, and the duration is clamped to the remaining media. Manual inspector input changes are clamped in the same manner.
- Timeline scroll buffer (Final Cut Pro style): To provide a comfortable workspace area at the end of the timeline, the layout calculates an extra timeline duration based on 40% of the timeline client container width divided by the current pixels-per-second scaling factor: `extraDuration = (scrollInfo.clientWidth * 0.4) / pxPerSecond`. The main rendering scope defines `timelineEnd` using `Math.max(5, totalDuration + extraDuration)` and `totalWidth` is calculated using `Math.max(1200, timelineEnd * pxPerSecond)`. Removing shadowed local declarations of `timelineEnd` ensures that grid background lines and time ruler ticks render correctly into this extended scroll buffer, maintaining design consistency across different zoom factors and viewport widths.
- Clip Splitting offsets: Splitting a clip into segment A and segment B sets `firstClip.trimEnd` and `secondClip.trimStart` to `target.trimStart + firstDuration`. This ensures that each segment plays only its respective portion of the original video or audio.
...
- Streaming Audio Extraction: Reading large video files (such as 24.4GB MOV files) directly as array buffers triggers browser out-of-memory errors due to Chrome's 2GB array buffer allocation limit. Using `mediabunny`'s `BlobSource` / `AudioBufferSink` allows chunk-by-chunk audio frame decoding to construct a WAV file, which scales successfully to very large projects without inflating the JS heap.
- Input vs. Output Seeking in FFmpeg: During reverse playback or scene change detection, seeking arguments (`-ss` and `-t`) must be placed before `-i` (input seeking) instead of after it (output seeking). Placing it before limits decoding and buffering strictly to the selected range. Placing it after forces FFmpeg to decode the entire video file first, causing WASM OOM crashes (exit code 1) on large assets.
- File Extension Preservation in FFmpeg WASM: FFmpeg's demuxer detects file formats using extensions and headers. Writing local files with a generic extension (e.g. `.mp4`) when they are actually `.mov` or `.webm` leads to `UnsupportedInputFormatError` or exit code 1. The original extension must be dynamically extracted and maintained in FFmpeg's virtual filesystem paths.
- Silent Video Fallbacks: Silent video files (no audio track) return `null` from audio extractors instead of throwing errors. Timeline waveforms are mapped to empty arrays (`[]`). When rendering previews or offline mixdowns, a 1-second silent `AudioBuffer` is generated to satisfy the web audio schedule logic, preventing canvas rendering freezes or playback stutters.
- Chunked File System Writing: Writing very large exported files in one continuous buffer via the File System Access API causes IPC communication overflows in Chrome, aborting the process (SIGABRT crash). Slicing the file into sequential 16MB chunks before writing to the `createWritable()` stream resolves this issue.
- Global AudioContext Sharing: Browsers restrict the number of concurrent active `AudioContext` instances (typically 6-10). Instantiating a new context for every timeline clip layer leads to context exhaustion and silences playback when clips are copied, pasted, duplicated, or unmounted. Sharing a single global context cached on `window` (`window.__creaibox_global_audio_context__`) across all MediaElement nodes ensures stable, leak-proof preview audio.
