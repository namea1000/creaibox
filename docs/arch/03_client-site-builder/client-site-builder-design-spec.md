...
- **문제점**: 과거 이관 엔진은 1차 텍스트 메뉴만 복제하거나, 2차 메뉴(드랍다운)가 복제되더라도 `htmlInjector.ts`가 서브페이지 링크를 강제로 덧붙이는 과정(Append)에서 수많은 링크가 세로로 뭉치는 레이아웃 파괴 현상이 발생했습니다.
- **아키텍처 적용 (Skip Injection)**:
  ...
  - `layout.tsx` 분기 처리: 이관 원본 사이트(`creation_source === "migration"`)일 경우, 원본 헤더에 이미 완벽한 메가 메뉴 링크가 내장되어 있으므로 **인위적인 동적 메뉴 주입 배열(`menusToInject`)을 비워버려(Empty Array) 주입(Injection)을 원천 스킵**합니다. 이를 통해 원본 2차/3차 다이내믹 메가 메뉴 디자인을 단 1%의 훼손도 없이 100% 보존합니다.

### 4.5.9 헤더 메뉴 좌측 쏠림 강제 교정 (Un-nesting)
- **문제점**: AI가 생성한 사이트나 일부 템플릿(예: 아임웹)에서 로고(`<a>`)와 메뉴(`<nav>`)를 동일한 부모 `<div>` 안에 묶어서 그룹핑(Grouping)해버려, 메뉴에 `mx-auto`를 부여해도 가운데 정렬이 되지 않고 좌측으로 찰싹 붙는 레이아웃 고질병 발생.
- **아키텍처 적용**:
  - `htmlInjector.ts` 런타임 엔진이 렌더링 직전에 개입하여, `<nav>`가 메인 헤더 컨테이너의 1뎁스 자식이 아닌 중첩된 박스에 갇혀있는 것을 감지하면 즉시 적출(Un-nesting)합니다.
  - 적출한 `<nav>`를 메인 헤더 컨테이너의 1뎁스 다이렉트 자식으로 강제 분리(Move)한 뒤 `mx-auto`를 주입함으로써, AI의 그룹핑 실수(환각)와 관계없이 **모든 사이트의 헤더 메뉴가 무조건 화면 정중앙에 배치**되는 글로벌 레이아웃 자동 방어 로직을 구축했습니다.

## 9. 대규모 사이트 무손실 이관 자동화 아키텍처 (Sequential Background Migration)

### 9.1 백그라운드 무손실 큐(Queue) 파이프라인
- **문제점**: 거대 사이트(서브페이지 15개 이상)를 한 번에 Gemini 모델에 프롬프트로 주입할 경우, 8,000 토큰 아웃풋 리밋(Output Limit)에 걸려 하단 섹션이 통째로 증발하거나 퀄리티가 현저히 저하되는 치명적 한계가 존재했습니다.
- **아키텍처 적용 (Queue & Cron Worker)**:
  - **1단계 (Main Page 우선 추출)**: 사용자가 이관을 요청하면 100% 퀄리티를 보장하기 위해 오직 '메인 페이지' 1장만 최우선으로 동기식(Synchronous) 렌더링하여 사용자에게 즉각 화면을 띄워줍니다. 나머지 서브페이지 URL 목록은 DB의 `client_sites.extra_configs.migration_queue` 배열에 임시 적재(Enqueue)합니다.
  - **2단계 (Worker Sequential Processing)**: 1분 주기로 백그라운드에서 깨어나는 Vercel Cron(`site-migration-worker`)이 큐에서 서브페이지를 1장씩 꺼내어(Dequeue) 프롬프트에 주입합니다. 모든 8,000 토큰의 역량을 단 한 장의 서브페이지에 100% 집중시켜 최고 화질로 렌더링하며 사이트를 점진적으로 완성합니다.

### 9.2 히스토리 폴링(Polling) 및 캐싱 방어 아키텍처
- **프론트엔드 실시간 프로그레스 (Live Progress)**: 백그라운드 이관이 진행되는 동안 유저가 지루함을 느끼지 않도록, 프론트엔드 대시보드에서 5초 단위로 API 핑(Ping)을 날려 큐의 상태(남은 페이지 수, 완료 퍼센티지, 예상 남은 시간)를 계산하고 화려한 애니메이션 프로그레스 바로 렌더링합니다.
- **Next.js 라우트 캐시 무력화**: `fetch` 기반 폴링이 Next.js 브라우저 캐시에 갇혀 상태가 갱신되지 않거나 백엔드 Worker가 호출되지 않는 현상을 방어하기 위해, 프론트엔드 Fetch에 `cache: "no-store"` 옵션을 부여하고 Worker 라우트에는 `export const dynamic = "force-dynamic"` 속성을 강제 주입하여 완벽한 실시간(Real-time) 동기화를 달성했습니다.

### 9.3 대상 웹사이트 정밀 스캔 (X-Ray Precision Scan)
- **동적 자원 실측 및 Vibe 분석**: 사용자가 "이관 시작"을 누르기 전, 대상 URL의 규모를 사전 파악할 수 있는 스크래퍼 API(`site-scan`)를 신설했습니다. 총 페이지 수, 텍스트 글자 수, 미디어 개수를 1초 만에 실측합니다.
- **Gemini 언어/톤앤매너 판별**: 스크래핑된 텍스트 덤프를 Gemini Flash 모델에 즉시 주입하여 타겟 사이트의 주 사용 언어와 디자인 분위기(Vibe)를 분석하고, 그 결과를 DB 정석 설계 규칙에 맞춰 `scan_report` JSONB 컬럼에 영구 보존하여 사용자 대시보드(Glassmorphism UI)에 브리핑합니다.
