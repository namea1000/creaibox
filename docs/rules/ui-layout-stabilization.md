...
## 4. 사이드바(Sidebar) UX 안정화 및 상태 영속성 규칙

### [문제점]
Next.js App Router 환경에서 최상위 레이아웃(Root Layout)이 다른 페이지로 이동할 경우, `<Sidebar>` 컴포넌트가 언마운트(Unmount) 후 재마운트(Remount)되는 현상이 발생합니다. 이로 인해 다음과 같은 심각한 UX 저하가 발생합니다.
* **메뉴 펼침 상태 초기화 (Auto-Expand Bug)**: 새로 렌더링되면서 현재 경로(Pathname)를 기반으로 서브 메뉴가 억지로 자동 전개되어, 사용자의 의도(클릭)와 무관하게 UI가 펼쳐짐.
* **권한 검증에 따른 메뉴 깜빡임 (Admin Menu Flicker)**: `isAdmin` 등 권한 상태가 초기화되어 관리자 전용 메뉴가 사라졌다가 수백ms 후 통신이 완료되면 다시 나타나는 깜빡임(Flicker) 발생.
* **현재 활성 메뉴 오작동 (Active Menu Click Bug)**: 이미 활성화된 메뉴를 클릭하여 서브메뉴를 열고자 할 때, 코드 상 경로 검증 로직에 의해 첫 번째 물리적 클릭이 두 번째 클릭(토글)으로 오인되어 의도치 않은 펼침/접힘이 발생.

### [해결 규칙]
1. **사이드바 펼침 상태 및 권한의 세션 스토리지 영속화 (Session Storage Persistence)**:
   * 사용자가 명시적으로 열고 닫은 서브 메뉴의 상태(`expandedGroups`)와 권한 상태(`isAdmin`)는 컴포넌트 내부 상태(`useState`)에 의존하지 않고, 반드시 브라우저 `sessionStorage`에 실시간으로 동기화해야 합니다.
   * 컴포넌트가 재마운트될 때는 `sessionStorage`에서 이전 상태를 가장 먼저 동기적으로 읽어와 초기값으로 사용함으로써 시각적 단절과 깜빡임을 완벽히 차단합니다.
   * _(단, 첫 진입 등 스토리지가 비어있을 경우에만 현재 경로를 기반으로 자동 전개되도록 허용)_

2. **사용자 의도 중심의 순수 물리적 클릭 추적 (Physical Click Tracking)**:
   * 사이드바 메뉴 클릭 시 현재 라우터 경로(URL)나 렌더링 상태(`isGroupActive`)에 의존하여 토글(Toggle) 여부를 결정해서는 안 됩니다.
   * 반드시 `lastClickedGroup`과 같이 사용자의 '물리적 클릭'을 독자적으로 추적하는 상태를 분리하여 운영하십시오.
   * **원칙**: 
     - 1차 클릭: 무조건 페이지 이동만 수행하며 서브 메뉴를 강제로 펼치지 않음 (`setLastClickedGroup(key)` 만 갱신).
     - 2차 클릭: 직전에 클릭한 메뉴(현재 활성 상태)를 재차 명확히 클릭했을 때만 서브 메뉴를 토글함 (`toggleGroup(key)` 실행).
     - (예외: 명시적인 꺾쇠(Chevron) 아이콘 클릭 시에는 횟수에 무관하게 즉시 토글)
