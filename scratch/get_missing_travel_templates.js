...
async function main() {
  ...
  // 3. Find missing Travel & Lifestyle templates
  // The category name might be "Travel & Lifestyle"
  const missingTravel = templates.filter(t => 
    (t.category === "Travel & Lifestyle" || t.category.includes("Travel")) && 
    !existingPreviews.has(t.templateId)
  );
  console.log(`Missing Travel & Lifestyle templates: ${missingTravel.length}`);
  console.log("Missing IDs:", missingTravel.map(t => t.templateId));

  if (missingTravel.length === 0) {
    console.log("No missing Travel templates!");
    return;
  }
  ...
  const targetIds = missingTravel.map(t => t.templateId);

  console.log("\n--- Details for Missing Travel Templates ---");
