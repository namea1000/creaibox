...
async function main() {
  try {
    ...
    const fullArticleWithCode = `서론: IT 뉴스 속 '엔진(Engine)'이란 대체 무엇인가?
...
[ 소스 코드 예시 1: 외부 CSS 숨겨진 이미지 전수 적출 파이프라인 ]
--------------------------------------------------------------------------------
// [TypeScript] CreaiBox CSS Deep Harvester Engine
import { extractLinkedStylesheets, fetchStylesheet } from "@/lib/server/scraper";

export async function harvestHiddenBackgrounds(targetHtml: string, origin: string) {
  const detectedImages = new Set<string>();
  const cssUrls = extractLinkedStylesheets(targetHtml, origin);

  // 1. 외부 CSS 파일을 병렬 스캔하여 background-image 속성 전수 추출
  await Promise.all(cssUrls.slice(0, 5).map(async (url) => {
    const cssContent = await fetchStylesheet(url);
    const bgMatches = cssContent.match(/url\\(['"]?([^'"()]+)['"]?\\)/gi) || [];
    bgMatches.forEach((match) => detectedImages.add(resolveToAbsolute(match, origin)));
  }));

  console.log(\`[CreaiBox Engine] 🎯 \${detectedImages.size}개의 숨겨진 고화질 배경 사진 100% 적출 완료!\`);
...
[ 소스 코드 예시 2: AI 매직 빌더 및 0초 글로벌 라이브 배포 ]
--------------------------------------------------------------------------------
// [TypeScript] AI Magic Builder & 0-Second Edge Live Deployment
const newSite = await creaiboxMagicBuilder.generate({
  industry: "프리미엄 레스토랑 & F&B",
  brandName: "더 테라스 가든",
  tone: "Luxury & Modern",
  engineMode: "PRO_CLONING_v1.20"
});

// ⚡ 클릭 즉시 전 세계 인터넷에 0초 만에 정식 배포
await deployToEdgeNetwork({
  subdomain: \`\${newSite.brandId}.creaibox.com\`,
  customDomain: "theterrace.kr",
  enableAutoSitemap: true,
  pingGoogleAndNaver: true
});
--------------------------------------------------------------------------------
...
[ 소스 코드 예시 3: 실시간 2배율 돋보기 & 360° 무한 회전 배지 ]
--------------------------------------------------------------------------------
// [React / TSX] Interactive Location Magnifier Component
<InteractiveLocationMagnifier
  mapImage="https://cdn.creaibox.com/assets/cheonan-map.webp"
  zoomImage="https://cdn.creaibox.com/assets/cheonan-zoom.webp"
  badgeText="CENTRAL LOCATION PREMIUM • "
  zoomFactor={2.0}
  enableSpinBadge={true}
  title="누리는 프리미엄의 클래스가 다른 세상"
  subtitle="Central Location"
/>
--------------------------------------------------------------------------------

[ 소스 코드 예시 4: 0.01초 수소폭탄 가속 서빙 스마트 링커 ]
--------------------------------------------------------------------------------
// [React / TSX] SmartIntentLink (0.01s Instant Subpage Navigation)
<SmartIntentLink
  href="/menu/signature"
  intentDelayMs={150}
  className="px-6 py-3 bg-[#2f2f4c] text-white font-bold rounded-2xl shadow-lg"
>
  시그니처 메뉴 보러가기 ➔
</SmartIntentLink>
--------------------------------------------------------------------------------
...
• 문의 및 사전 알림 신청: creaibox.com 공식 홈페이지`;
    ...
    console.log("Successfully updated Google Docs with code snippets!");
  } catch (err: any) {
