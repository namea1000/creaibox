...
async function updateSheetHeaderAndData() {
  const auth = getAuthClient();
  const sheets = google.sheets({ version: "v4", auth });
  const spreadsheetId = "1cI6-XYJKAYtaTSL97X8ryOaast7vIGoGR892dx7S59I";

  console.log("Re-aligning sheet headers for 10 styles (3 original + 7 new)...");

  // 1. 헤더 수정 및 데이터 업데이트 (총 13열: A ~ M)
  const headers = [
    [
      "1차 대분류 카테고리 (Primary)", 
  ...
  // 기존 시트 전체 데이터 조회를 통해 카테고리 정보 로드
  const getRes = await sheets.spreadsheets.values.get({
    spreadsheetId,
    range: "시트1!A2:C19"
  });
  
  const currentCategories = getRes.data.values || [];

  // 각 로우 조립 (카테고리 A,B,C + 확장 프롬프트 D ~ M)
  const rows = currentCategories.map((cat, index) => {
    const data = expandedPrompts[index] || { 
      photo: "", illust: "", vector: "", render3d: "", anime: "", pixel: "", 
      watercolor: "", lineart: "", pattern: "", popart: "" 
    };
    return [
      cat[0] || "",
      cat[1] || "",
  ...
  console.log("Writing fully expanded dataset to Google Sheet...");
  ...
  console.log("Sheet structure has been upgraded with 10 styles successfully!");
}

updateSheetHeaderAndData().catch(err => {
  console.error("Upgrading sheet failed:", err);
});
