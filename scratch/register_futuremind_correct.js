...
async function registerFuturemind() {
  ...
  if (existing) {
    console.log("Updating futuremind in client_sites...");
    await supabase.from('client_sites').update(payload).eq('id', existing.id);
  } else {
    console.log("Inserting futuremind in client_sites...");
    await supabase.from('client_sites').insert(payload);
  }
  console.log("✅ futuremind registered in client_sites!");
}
