...
async function main() {
  ...
  // 3. Find missing Magazine templates
  const missingMagazine = templates.filter(t => t.category === "Magazine" && !existingPreviews.has(t.templateId));
  console.log(`Missing Magazine templates: ${missingMagazine.length}`);
  console.log("Missing Magazine IDs:", missingMagazine.map(t => t.templateId));

  if (missingMagazine.length === 0) {
    console.log("No missing Magazine templates!");
    return;
  }
  ...
  const targetIds = missingMagazine.map(t => t.templateId);

  console.log("\n--- Details for Missing Magazine Templates ---");
