...
function test(html, name) {
  ...
  if (navContainer.length > 0) {
    // Robust un-nesting:
    // 1. Find the "Header Row": The closest ancestor that has `justify-between` OR is the `header` itself if it's flex, 
    //    or the highest level div inside header that contains the nav.
    let headerRow = navContainer.closest('.justify-between');
    if (headerRow.length === 0) {
       // If no justify-between, maybe the header itself is the row, or a wrapper inside it
    ...
    if (headerRow.length > 0) {
       ...
       if (parent.get(0) !== headerRow.get(0)) {
         ...
         if (ancestor.parent().get(0) === headerRow.get(0)) {
            // Is ancestor the FIRST child (left side) or LAST child (right side)?
            // If it's the right side (like abocado), extracting it before the right side will center it!
            // If it's the left side (like clickn), extracting it after the left side will center it!
            
            // Wait, is ancestor the left or right side?
            // If it's the left side (previous sibling doesn't exist or is small), we insert AFTER ancestor.
            // If it's the right side (next sibling doesn't exist), we insert BEFORE ancestor.
    ...
    // Check if the logo is now the only thing left in its wrapper. If so, give the nav mx-auto.
    // Actually, mx-auto is always good.
    navContainer.addClass('mx-auto');
    // If we want it strictly centered, we might need flex-1 on the left and right siblings.
  }
