...
export default function WorldContent() {
  ...
  // 🌟 세계/국제 뉴스 전용 엔진 가동 (함수명 명확하게 수정)
  const fetchWorldNews = async () => {
    setIsLoading(true);
    try {
      const response = await fetch('/api/news?keyword=' + encodeURIComponent('세계 국제 외신 미국 유럽 아시아'));
      const data = await response.json();
      setNews(data);
    } catch (error) {
      console.error("세계 뉴스 로드 실패:", error);
    } finally {
      setIsLoading(false);
    }
  };
    ...
    return (
      <div className="p-20 flex flex-col items-center justify-center space-y-4 min-h-[500px]">
        ...
        <p className="text-zinc-500 text-sm font-black uppercase tracking-[0.3em] italic">
          Syncing Global Intel... {/* 🌟 빌드 오류를 뿜던 IT 과학 문구를 세계 뉴스로 정교화 교정! */}
        </p>
      </div>
    );
  }
      ...
      <div className="flex items-center justify-between border-b border-zinc-800/80 pb-6">
        <div className="space-y-1">
          <h3 className="text-2xl font-black text-white italic uppercase tracking-tight flex items-center gap-2">
            <Layers size={20} className="text-cyan-500" /> GLOBAL <span className="text-cyan-500">NEWSROOM</span>
          </h3>
          <p className="text-zinc-500 text-[11px] font-bold uppercase tracking-widest">
            미국, 유럽, 아시아 등 외신 연동 실시간 국제 정세 분석 피드
          </p>
        </div>
        <button 
          onClick={fetchWorldNews}
          className="p-2.5 bg-zinc-900 border border-zinc-800 rounded-xl hover:border-cyan-500/50 text-zinc-400 hover:text-white transition-all"
        >
          <RefreshCcw size={16} />
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {news.map((item, index) => (
          <div 
            ...
            className="group flex gap-5 p-5 bg-zinc-900/20 border border-zinc-800/60 rounded-[28px] hover:border-cyan-500/40 hover:bg-zinc-900/40 transition-all duration-300 cursor-pointer shadow-xl relative overflow-hidden"
          >
            {/* 뉴스 대표 이미지 */}
            <div className="w-28 h-24 lg:w-36 lg:h-24 bg-zinc-800 rounded-2xl overflow-hidden shrink-0 relative border border-zinc-800">
              <img 
                ...
                loading="lazy"
                onError={(e) => {
                  e.currentTarget.src = 'https://images.unsplash.com/photo-1451187580459-43490279c0fa?q=80&w=500'; // 🌟 글로벌 스케일에 맞는 우주/지구 대체 이미지 매칭
                }}
              />
            </div>
            ...
            <div className="flex flex-col justify-between flex-1 min-w-0 py-1">
              <div className="space-y-1.5">
                <span className="text-[10px] font-black text-cyan-400 uppercase tracking-widest bg-cyan-500/10 px-2 py-0.5 rounded-md">
                  {item.source}
                </span>
        ...
        >
          <button 
            ...
            className={`absolute left-4 lg:left-8 p-3 bg-zinc-900/80 border border-zinc-800 text-white rounded-full transition-all z-20 ${
              selectedIndex === 0 ? 'opacity-20 cursor-not-allowed' : 'hover:bg-cyan-600 hover:border-cyan-500'
            }`}
          >
            <ChevronLeft size={28} />
          </button>
          ...
          >
            <div className="h-14 bg-zinc-900/50 border-b border-zinc-800/80 px-6 flex items-center justify-between">
              <div className="flex items-center gap-3 min-w-0">
                <span className="px-2.5 py-0.5 bg-cyan-500 text-black text-[10px] font-black rounded-md shrink-0">
          ...
          <button 
            ...
            className={`absolute right-4 lg:right-8 p-3 bg-zinc-900/80 border border-zinc-800 text-white rounded-full transition-all z-20 ${
              selectedIndex === news.length - 1 ? 'opacity-20 cursor-not-allowed' : 'hover:bg-cyan-600 hover:border-cyan-500'
            }`}
          >
