  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        answer: "네이버 검색 결과에 내 블로그/홈페이지를 노출하고 수집 현황 및 클릭수를 직접 확인하시려면 네이버 서치어드바이저 소유권 확인이 필요합니다.\n\n[네이버 소유권 확인 4단계 가이드]\n1. 네이버 서치어드바이저(searchadvisor.naver.com)에 로그인 후 [웹마스터 도구]로 이동하여 내 블로그 주소(https://내브랜드.creaibox.com 또는 https://독립도메인)를 입력합니다. (💡 꿀팁: http:// 대신 보안 인증서(SSL)가 기본 적용된 **https://** 로 등록하셔야 네이버 검색 품질 지수 및 상위 노출에 훨씬 유리합니다!)\n2. 사이트 소유확인 방법 중 'HTML 태그'를 선택하고 메타태그 전체(예: <meta name=\"naver-site-verification\" content=\"...\" />)를 복사하거나, content=\"...\" 안의 영숫자 키 값만 복사합니다.\n3. 크리에이박스 스튜디오 [크리에이박스 블로그 -> 블로그 설정 및 관리 -> SEO 및 연동 관리] 탭으로 이동합니다.\n4. '네이버 서치어드바이저 연동 키' 입력란에 복사한 값을 붙여넣고 저장합니다. (💡 영숫자 키 값만 넣으셔도 되고, <meta ...> 태그 전체를 그대로 붙여넣으셔도 시스템이 알아서 키를 자동 추출하므로 둘 다 100% 정상 연동됩니다!)\n5. 네이버 서치어드바이저 화면으로 돌아와 [소유확인] 버튼을 누르면 즉시 인정됩니다.\n6. 소유확인 완료 후 [요청 -> 사이트맵 제출] 메뉴에서 sitemap.xml 을 제출하고, [요청 -> RSS 제출] 메뉴에서 전체 피드 주소(https://내브랜드.creaibox.com/feed)를 등록해 주시면 네이버 검색 로봇 수집이 대폭 빨라집니다.",
        link: "/studio/writing/creaibox/blog-management",
      ...
      },
      {
        ...
        answer: "구글 검색 노출 키워드와 포스팅별 클릭수/조회수 분석을 위해 본인의 구글 계정으로 서치콘솔 소유권을 인증하실 수 있습니다.\n\n[구글 서치콘솔 등록 4단계 가이드]\n1. 구글 서치콘솔(search.google.com/search-console) 접속 후 로그인합니다.\n2. 속성 유형 선택 화면에서 'URL 접두사' 방식을 선택하고 내 블로그 주소(https://내브랜드.creaibox.com 또는 https://독립도메인)를 입력합니다. (💡 꿀팁: http:// 대신 보안 인증서(SSL)가 기본 적용된 **https://** 로 등록하셔야 구글 검색 랭킹에 훨씬 유리합니다!)\n3. 소유권 확인 방법으로 'HTML 태그' 방식을 선택하고 메타태그 전체(예: <meta name=\"google-site-verification\" content=\"...\" />) 또는 content=\"...\" 안의 키 값을 복사합니다.\n4. 크리에이박스 스튜디오 [크리에이박스 블로그 -> 블로그 설정 및 관리 -> SEO 및 연동 관리] 탭의 '구글 서치콘솔 연동 키' 입력란에 붙여넣고 저장합니다. (💡 영숫자 키 값만 넣으셔도 되고, <meta ...> 태그 전체를 그대로 붙여넣으셔도 시스템이 알아서 키를 자동 추출하므로 둘 다 100% 정상 연동됩니다!)\n5. 구글 서치콘솔 화면에서 [확인] 버튼을 클릭하면 소유권 검증이 즉시 완료됩니다.\n6. 인증 완료 후 서치콘솔 [Sitemaps] 메뉴에서 sitemap.xml 과 feed (RSS 피드 주소)를 각각 입력하여 제출해 주시면 구글 크롤링 로봇이 신규 포스팅을 실시간 감지합니다.",
        link: "/studio/writing/creaibox/blog-management",
        linkLabel: "SEO 및 연동 관리 페이지로 이동"
      }
    ]
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        id: "trbl-3",
        category: "troubleshoot",
        question: "웹사이트 접속 시 '이 기기의 다른 앱 및 서비스에 액세스' 권한 팝업이 뜨는데 왜 그런가요?",
        answer: "해당 팝업은 크롬(Chrome) 및 엣지(Edge) 브라우저가 최신 보안 간편 로그인(구글 One-Tap 및 패스키/Passkey) 기능 연동을 확인하기 위해 브라우저 자체에서 띄우는 정상적인 시스템 보안 안내 창입니다.\n\n* **문구의 의미**: 기기(PC)에 저장된 구글 계정이나 간편 로그인 자격증명을 사이트에서 안전하게 조회해도 되는지 확인하는 절차이며, PC의 다른 프로그램이나 개인 파일에 무단 접근하는 것이 아니므로 안심하셔도 됩니다.\n* **[허용] 선택 시**: 비밀번호 입력 없이 빠르고 편리한 원클릭 패스키/소셜 간편 로그인이 지원됩니다.\n* **[차단] 선택 시**: 차단 또는 닫기를 누르시더라도 블로그 글 열람 및 모든 웹사이트 기능 이용에는 아무런 제약이 없으며 100% 정상 작동합니다.\n* **노출 횟수**: 새 PC/브라우저 환경에서 최초 1회만 노출되며, 한 번 선택하시면 동일 브라우저에서는 다시 나타나지 않습니다.",
        link: "/help",
        linkLabel: "고객지원 FAQ 센터 이동"
      }
    ]
  }
];
