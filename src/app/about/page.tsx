...
export default function AboutPage() {
  ...
  const coreValues = [
    ...
    },
    {
      ...
      description: "CreAI Labs의 기술력을 바탕으로 강력한 API 보안 및 데이터 관리 생태계를 구축하여 기업과 개인 창작자 모두 안심하고 사용할 수 있는 환경을 만듭니다."
    }
  ];
      ...
      <div className="max-w-6xl mx-auto px-6 lg:px-12 py-20 relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto space-y-6 mb-24">
          ...
          <h1 className="text-4xl md:text-5xl lg:text-6xl font-black tracking-tighter leading-tight">
            상상력을 현실로 만드는 <br />
            가장 똑똑한 <span className="bg-gradient-to-r from-blue-500 to-indigo-400 bg-clip-text text-transparent">AI 상자</span>
          </h1>
          <p className="text-base md:text-lg text-zinc-400 font-medium leading-relaxed pt-2">
            <span className="text-zinc-200 font-bold">CreAI Labs</span>가 선보이는 올인원 AI 콘텐츠 생성 플랫폼, 크리에이아이박스입니다. 우리는 번거로운 창작 프로세스를 혁신하여 모든 이들이 제약 없이 크리에이티브를 펼치는 미래를 설계합니다.
          </p>
        </div>

        {/* 💻 SECTION 2: PLATFORM IDENTITY (대형 터미널 포스 비주얼 박스) */}
        <div className="w-full rounded-2xl border border-zinc-800/80 bg-zinc-900/40 backdrop-blur-md p-6 md:p-8 mb-28 shadow-2xl relative">
          <div className="flex items-center gap-2 mb-6 border-b border-zinc-800/60 pb-4">
            <div className="w-3 w-3 h-3 rounded-full bg-red-500/70" />
            <div className="w-3 w-3 h-3 rounded-full bg-yellow-500/70" />
            <div className="w-3 w-3 h-3 rounded-full bg-green-500/70" />
            <span className="text-xs font-mono text-zinc-500 ml-2 flex items-center gap-1">
              <Terminal size={12} /> core-mission.sh
            </span>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
            <div className="space-y-4">
              <h3 className="text-xl md:text-2xl font-extrabold tracking-tight text-white flex items-center gap-2">
                <Layers className="text-blue-500" size={20} /> 하나의 Box 안에서 끝나는 올인원 스튜디오
              </h3>
              <p className="text-sm text-zinc-400 leading-relaxed font-medium">
                우리는 글쓰기, 이미지, 영상, 음악이 각각 파편화되어 있는 기존 AI 툴들의 불편함에 주목했습니다. 크리에이아이박스는 단 하나의 플랫폼 안에서 콘텐츠 기획부터 멀티미디어 제작, 트렌드 분석 리포트까지 유기적으로 연결되는 완벽한 워크플로우를 제공합니다.
              </p>
            </div>
            <div className="bg-zinc-950/80 rounded-xl p-5 border border-zinc-850 font-mono text-xs md:text-sm text-blue-400/90 space-y-2 shadow-inner">
              ...
              <p><span className="text-zinc-600">5</span> &nbsp;&nbsp;<span className="text-purple-400">return</span> coreTech.map(tech =&gt; <span className="text-white">`Build_$&123;tech&125;_With_AI`</span>);</p>
              <p><span className="text-zinc-600">6</span> &#125;</p>
              <p><span className="text-zinc-600">7</span> <span className="text-zinc-500">// Result: 상상이 현실이 되는 최단 거리 확정</span></p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-12 mb-28">
          <div className="text-center space-y-3">
            <h2 className="text-2xl md:text-3xl font-black tracking-tight flex items-center justify-center gap-2">
              <Target className="text-blue-500" size={22} /> Our Core Values
            </h2>
            <p className="text-sm text-zinc-400 font-medium">크리에이아이박스가 양보하지 않는 세 가지 신념입니다.</p>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {coreValues.map((value, idx) => (
              <div 
                key={idx} 
                className="p-6 md:p-8 rounded-2xl border border-zinc-800/40 bg-zinc-900/20 transition-all duration-300 hover:border-zinc-700/60 hover:bg-zinc-900/40 group shadow-lg"
              >
                <div className="w-12 h-12 rounded-xl bg-zinc-900 border border-zinc-800 flex items-center justify-center mb-6 group-hover:scale-110 transition-transform">
                  {value.icon}
                </div>
                <h4 className="text-base font-black text-zinc-100 mb-3 tracking-tight">
                  {value.title}
                </h4>
        ...
        <div className="w-full rounded-2xl border border-blue-500/20 bg-gradient-to-br from-blue-950/20 to-zinc-950 p-8 md:p-12 text-center space-y-6 shadow-2xl relative overflow-hidden">
          ...
          <p className="text-sm text-zinc-400 font-medium max-w-xl mx-auto leading-relaxed">
            비즈니스 마케터, 1인 크리에이터, 블로거, 그리고 상상을 멈추지 않는 모든 이들을 위해 준비되어 있습니다. 크리에이아이박스의 인프라를 직접 경험해 보세요.
          </p>
          <div className="pt-4">
            <Link href="/studio/writing">
              <button className="px-6 py-3 bg-blue-600 hover:bg-blue-500 text-white text-sm font-black rounded-xl shadow-lg active:scale-95 transition-all inline-flex items-center gap-2 group">
