...
import { Globe, Settings, Eye, Phone, MapPin, Loader2, Plus, LayoutDashboard, Wand2, Palette, MessageSquare, FileText, ChevronRight, X } from "lucide-react";
...
function SiteBuilderLayoutContent({ children }: { children: React.ReactNode }) {
  ...
  // 2. Enforce Pro/Premiere/Business/Enterprise/Admin Membership Restrictions for LOGGED IN users
  if (profile) {
  ...
  // Create New Site Trigger (With Auth Guard)
  ...
  const hasSites = profile && sites && sites.length > 0 && selectedSite;
      ...
      <div className="w-full max-w-[1720px] mx-auto p-4 sm:p-6 md:p-8 space-y-8 animate-fade-in">
        ...
        {showLoginModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 animate-fade-in">
            <div className="bg-white dark:bg-[#0b0f19] border border-slate-200 dark:border-slate-800/90 rounded-3xl p-8 max-w-md w-full text-center space-y-6 shadow-2xl relative overflow-hidden animate-fade-in-up">
              <button
                onClick={() => setShowLoginModal(false)}
                className="absolute top-4 right-4 text-slate-400 hover:text-slate-600 dark:hover:text-white p-1 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
              >
                <X size={18} />
              </button>
              ...
              <div className="space-y-3 pt-2">
                ...
                <button
                  onClick={() => setShowLoginModal(false)}
                  className="w-full inline-flex items-center justify-center gap-2 px-6 py-3 text-xs font-bold text-slate-600 dark:text-slate-400 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 rounded-xl transition-all cursor-pointer"
                >
                  <span>둘러보기 계속하기</span>
                </button>
              </div>
          ...
          <div className="bg-white dark:bg-[#0b0f19] border border-slate-200 dark:border-slate-800/90 rounded-3xl p-8 max-w-md w-full text-center space-y-6 shadow-2xl relative overflow-hidden animate-fade-in-up">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 font-semibold leading-relaxed">
                ...
                로그인 후 1초 만에 나만의 맞춤형 비즈니스 웹사이트를 만들어 보세요!
              </p>
            </div>
