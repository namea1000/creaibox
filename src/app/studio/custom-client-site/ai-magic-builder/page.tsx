...
import AiMagicBuilderTab from "@/components/studio/custom-client-site/tabs/AiMagicBuilderTab";

export default function SnsBuilderPage() {
  ...
  return (
    <div className="min-h-screen bg-[#0d0f14] text-slate-100 font-sans p-6 lg:p-10">
      <div className="mb-6">
        <h1 className="text-2xl font-black text-white">AI 홈페이지 매직 빌더 🪄</h1>
        <p className="text-sm text-slate-400 mt-1">홈페이지가 없으신가요? 블로그 등 운영 중인 채널의 URL이나 참조 문서(텍스트, PDF)를 입력하면 AI가 비즈니스를 분석해 기적처럼 홈페이지를 창작해 드립니다.</p>
      </div>

      <AiMagicBuilderTab requireAuth={requireAuth} />
