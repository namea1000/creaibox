...
import { Search, AlertCircle, RefreshCw, Trash2, Globe, Edit3, Hash } from 'lucide-react';
import { useQueryClient } from '@tanstack/react-query';
import { createClient } from '@/utils/supabase/client';
import { useCreaiboxManuscriptsQuery, creaiboxManuscriptKeys, type StudioManuscriptRecord } from '@/lib/queries/manuscripts';
...
export default function CreaiboxManuscriptListPage() {
  ...
  const { data: manuscripts = [], isLoading, isFetching, refetch } = useCreaiboxManuscriptsQuery();
  ...
  const handleOpenManuscript = useCallback((manuscript: StudioManuscriptRecord, index: number) => {
  ...
  const handlePublish = useCallback(async (manuscript: StudioManuscriptRecord) => {
    ...
    const { error } = await supabase
      ...
      .update({ status: 'published', canonical_url: manuscript.canonicalUrl || `https://creaibox.com/blog/${manuscript.slug || manuscript.id}` })
      .eq('id', manuscript.id);
    ...
    <div className="min-h-screen bg-[#0a0d12] text-white px-6 py-6">
      ...
      <div className="mb-5 rounded-2xl border border-white/10 bg-[#10141c] px-5 py-4 text-white/70 flex items-center justify-between">
        ...
        >
          ...
          {isFetching ? '최신 목록 동기화 중...' : '글 목록 가져오기 (수동 새로고침)'}
        </button>
      </div>
      ...
      <div className="overflow-hidden rounded-3xl border border-white/10 bg-[#0d1118]">
        ...
        <div>
          {paddedRows.map((manuscript, index) => {
            ...
            const modeLabel = manuscript.sourceMode === 'url' ? 'URL 재창조' : manuscript.sourceMode === 'text' ? '텍스트 재창조' : manuscript.postType === 'recreate' ? 'AI 재창조' : 'AI 인사이트 포스팅';
            const isSelected = pathname === `/studio/writing/creaibox/list/${getDisplayId(manuscript, index)}`;
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center justify-end gap-2" onClick={(event) => event.stopPropagation()}>
                  <button onClick={() => handleOpenManuscript(manuscript, index)} className="rounded-full border border-white/10 bg-white/5 p-3 text-white/80 hover:bg-white/10"><Edit3 className="h-4 w-4" /></button>
