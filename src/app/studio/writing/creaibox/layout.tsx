...
export default function CreaiboxBlogLayout({ children }: { children: React.ReactNode }) {
  ...
  const tabs = [
    { id: 'home', label: '블로그 홈', icon: LayoutGrid, color: 'text-blue-400', href: '/studio/writing/creaibox' },
    ...
    { id: 'list', label: '발행 원고 관리', icon: FileEdit, color: 'text-indigo-400', href: '/studio/writing/creaibox/list' },
    ...
    <div className="flex flex-col min-h-screen w-full bg-[#050505] text-zinc-100 font-sans">
      ...
      <div className="flex flex-1 pt-20 overflow-hidden">
        ...
        <main className="flex-1 flex flex-col min-w-0 border-l border-zinc-800 bg-[#050505] overflow-y-auto custom-scrollbar">
          ...
          <div className="sticky top-0 z-20 flex items-center px-4 border-b shrink-0 bg-[#0a0a0a]/90 backdrop-blur-xl border-zinc-800 shadow-[0_4px_20px_rgba(0,0,0,0.5)]">
            <div className="flex overflow-x-auto no-scrollbar py-2 w-full gap-2">
              {tabs.map((tab) => {
                ...
                return (
                  <Link
                    key={tab.id}
                    href={tab.href}
                    className={`flex items-center gap-2 px-4 py-2 text-sm font-bold uppercase tracking-wider transition-all rounded-lg border ${
                      isActive 
                        ? 'text-white bg-blue-600/20 border-blue-500/50 shadow-[0_0_15px_rgba(37,99,235,0.3)]' 
                        : 'text-zinc-500 hover:text-zinc-200 hover:bg-zinc-900 border-transparent'
                    }`}
                  >
                    <tab.icon size={16} className={isActive ? 'text-blue-400' : tab.color} />
                    {tab.label}
                  </Link>
                );
              })}
          ...
          <div className="flex-1 w-full mx-auto p-2 md:p-3">
            {children}
          </div>
        </main>
