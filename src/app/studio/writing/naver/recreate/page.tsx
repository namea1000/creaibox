  ...
  const handleAiRecreate = async () => {
    ...
    try {
      ...
      const prompt = `
        ...
        2. 🌟 본문(content) 맨 최상단에 [추천 변형 제목 4선] 필수 생성:
           원고 본문(content)의 1행에 사용자가 골라서 제목으로 쓸 수 있도록 C-Rank / DIA+ 검색 알고리즘 및 유사문서 회피에 최적화된 추천 제목 4가지를 아래 양식대로 정확히 작성하라:
           💡 [네이버 C-Rank / DIA+ 상위노출 추천 제목 4선]
           1️⃣ (이슈/클릭 유발형): [변형 제목 1]
           2️⃣ (스펙/전문성 분석형): [변형 제목 2]
           3️⃣ (비교/차별화 강조형): [변형 제목 3]
           4️⃣ (친근한 블로그 후기형): [변형 제목 4]
           (주의: 추천 제목 4선과 본문 사이에는 절대로 구분선(---)을 넣지 말고 공백 한 줄로 연결하라.)
        3. 최종 선정한 타겟 키워드를 본문 안에 3회~5회 내외로 자연스럽게 배치하라.
        4. 말투는 반드시 '${selectedTone}'에 맞춰 유지하라.
        5. ${lengthInstruction}
        6. 마크다운의 대제목 및 소제목 구조(##, ###)를 반드시 3개 이상 쪼개어 가독성 벨트를 형성하라.
        7. 🌟 네이버 SEO 태그 필수 생성: 본문(content)의 맨 마지막 줄(최하단)에는 원고 내용과 연관된 네이버 검색 전용 태그 5개~6개를 "#태그1, #태그2, #태그3, #태그4, #태그5" 포맷으로 한 줄에 정확히 작성하여 반드시 포함시켜라. (예시: #문샷AI키미, #중국AI기술동향, #KimiK3성능, #대규모컨텍스트AI, #중국생성형AI모델)
        8. 동시에 원본 글의 핵심 키워드, 핵심 주제, 핵심 내용을 사람이 한눈에 파악할 수 있게 별도 분석하라.
        9. 결과물은 부연설명이나 마크다운 코드 블록 선언부 기호 없이 오직 순수한 JSON 형식 데이터 규격으로만 정확하게 배출하라.
