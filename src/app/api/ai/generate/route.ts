...
function getFriendlyAiErrorMessage(error: unknown) {
  ...
  if (!errMsg) return "AI 생성 중 문제가 발생했습니다. 잠시 후 다시 시도해주세요.";
  
  if (errMsg.toLowerCase().includes("prepayment credits") || errMsg.toLowerCase().includes("depleted") || errMsg.toLowerCase().includes("billing")) {
  ...
  if (isHighDemandError(error)) {
    return "AI 서버가 현재 혼잡합니다. 잠시 후 다시 시도해주세요.";
  }

  return `AI 생성 오류: ${errMsg}`;
}
