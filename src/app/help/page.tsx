...
export default function HelpCenterPage() {
  ...
  const faqList = [
    ...
    {
      category: "ai",
      question: "내 브랜드 블로그가 구글/네이버 검색에 자동으로 노출되나요? (서치콘솔 연동 안내)",
      answer: "사용하시는 블로그 도메인의 형태에 따라 다릅니다.\n\n1. 기본 서브도메인 사용 고객 (아이디.creaibox.com): 대표 메인 사이트의 하위 서브 도메인으로 취급되어 사용자가 직접 구글 서치콘솔이나 네이버 서치어드바이저에 개설 및 소유권 인증을 하실 필요가 전혀 없습니다! 글을 쓰시는 즉시 검색 로봇이 자동으로 찾아와 인덱싱합니다.\n2. 개인 독립 도메인 사용 고객 (예: 본인도메인.com): 독립된 사유 도메인이므로 최초 1회 구글 서치콘솔/네이버 서치어드바이저에 등록하셔야 합니다. [블로그 관리 > SEO 및 연동 관리] 메뉴에 메타태그 인증키를 복사-붙여넣기하여 소유권을 확인한 후, 본인의 사이트맵(https://도메인/sitemap.xml)과 RSS 피드(https://도메인/feed)를 한 번만 제출해 주시면 그 이후부터는 영구히 자동 배달됩니다."
    },
    {
      category: "ai",
      question: "구글 애널리틱스(GA4)를 내 블로그에 직접 가입해서 수동으로 연동해야 하나요?",
      answer: "아닙니다! 크리에이박스는 구글 애널리틱스 연동 과정을 100% 무인 자동화했습니다.\n\n블로그 개설 및 독립 도메인이 연결 승인되는 즉시, 크리에이박스 백엔드 서버가 Google Analytics Admin API를 직접 호출하여 전용 웹 스트림과 측정 ID(G-XXXXXX)를 자동 생성해 줍니다. 그리고 블로그 사이트 헤더에 자동으로 추적 코드를 주입하므로, 사용자는 구글 애널리틱스 사이트에 가입하여 추적 코드를 긁어오는 번거로운 설정을 일절 하실 필요가 없습니다."
    },
    {
      category: "ai",
      question: "내 블로그의 사이트맵(sitemap.xml)과 RSS 피드(/feed)를 왜 직접 검색엔진에 제출해야 하나요? 방법은 무엇인가요?",
      answer: "내 블로그에 작성한 글을 구글과 네이버 검색 결과에 수 시간 이내로 가장 빠르고 누락 없이 노출시키기 위한 필수 조치입니다.\n\n[직접 제출해야 하는 이유]\n1. 시스템(본사)이 대행할 수 없는 영역: 구글 서치콘솔과 네이버 서치어드바이저는 보안 및 어뷰징 방지를 위해 반드시 '블로그 실소유자 본인의 개인 계정'으로 로그인하여 소유권 인증 및 사이트맵 제출을 받기 때문입니다.\n2. 실시간 고속 수집 개통: 지도를 등록하지 않아도 검색엔진 봇이 링크를 타고 흘러 들어와 글을 드문드문 긁어갈 수 있지만, 사이트맵과 피드를 정식 제출해 두면 봇이 매일 최우선적으로 가장 신속하게 새 글을 색인(Index)해 갑니다.\n\n[제출 및 등록 방법]\n1. 먼저 본인 블로그의 주소 규격에 맞는 사이트맵과 피드 주소를 확보합니다:\n- 사이트맵 주소: https://[내블로그도메인]/sitemap.xml (블로그 관리 > SEO 탭에서 클릭 한 번으로 복사 가능)\n- RSS 피드 주소: https://[내블로그도메인]/feed\n2. 구글 서치콘솔 제출:\n- 구글 서치콘솔 로그인 후 좌측 [Sitemaps] 메뉴로 이동합니다.\n- '새 사이트맵 추가' 입력란에 sitemap.xml 과 feed 를 각각 입력한 뒤 [제출]을 클릭합니다.\n3. 네이버 서치어드바이저 제출:\n- 네이버 웹마스터 도구 로그인 후 도메인 관리 화면으로 들어갑니다.\n- 좌측 [요청] > [사이트맵 제출] 메뉴에서 sitemap.xml 을 입력하여 제출합니다.\n- 좌측 [요청] > [RSS 제출] 메뉴에서 https://[내블로그도메인]/feed 전체 주소를 정확히 입력하여 제출합니다."
    },
