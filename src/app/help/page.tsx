...
export default function HelpCenterPage() {
  ...
  const faqList = [
    ...
    {
      category: "video",
      question: "무료 공유 에셋 비디오를 유튜브(쇼츠)에 올릴 때 저작권이나 '재사용 콘텐츠' 제재 우려가 없나요?",
      answer: "크리에이박스의 모든 무료 공유 에셋은 상업적 가공이 허용된 안심 라이선스로 제공되므로 단순 저작권 침해나 법적 문제는 발생하지 않습니다.\n다만, 다운로드한 비디오 클립을 아무런 편집 없이 유튜브에 그대로 업로드하는 경우, 유튜브 알고리즘이 이를 타인과 동일한 복제물로 판단하는 '재사용된 콘텐츠(Reused Content)' 필터에 걸려 채널의 수익 창출이 보류되거나 제한될 수 있습니다.\n이를 방지하기 위해, 반드시 크리에이박스 비디오 에디터를 사용하여 '나만의 자막/텍스트 자막 배치', 'AI 성우 음성 또는 직접 녹음한 나레이션 추가', '독창적인 컷 편집 및 배경음악 합성' 등 2차 가공을 가미하여 독창적인 가치를 불어넣어 업로드하실 것을 강력히 권장합니다."
    },
    {
      category: "video",
      question: "비디오 스튜디오에서 음악 편집 시 WAV 고해상도 파일과 MP3 파일의 차이점이 있나요? 무엇을 사용해야 하나요?",
      answer: "시스템의 안정성과 빠른 편집 속도를 위해 음악 작업 시 .mp3 파일 사용을 강력히 권장합니다. 상세한 차이점은 아래와 같습니다.\n\n1. 최종 영상 용량 차이: 없습니다. 영상 내보내기(Export)를 실행하면 타임라인에 삽입된 오디오는 원본 포맷과 무관하게 최종 인코딩 설정(예: AAC 160kbps)으로 재압축되어 영상 파일에 병합되므로 최종 비디오 파일 용량은 동일합니다.\n\n2. 브라우저 메모리(RAM) 점유율: 디코딩된 시점에는 차이가 없습니다. Web Audio API는 음악을 연주하거나 처리할 때 원본 포맷의 압축을 완전히 풀어 Float32 실수형 배열로 메모리에 로드하므로, 재생 시간과 주파수가 같다면 MP3와 WAV 모두 동일한 메모리 크기를 점유합니다.\n\n3. 결정적인 차이점 (편집 중 보존 용량): 비디오 스튜디오는 새로고침이나 브라우저 재시작 시 편집 상태를 유지하기 위해 사용자가 올린 원본 미디어 파일을 브라우저 로컬 데이터베이스(IndexedDB)에 백업해 둡니다. WAV 파일은 MP3 파일보다 용량이 10배 이상 크기 때문에, 대용량 WAV를 여러 개 사용할 경우 브라우저 저장소 제한 한계(Quota Limit)에 도달해 저장 실패 오류가 발생할 수 있습니다. 또한, 백업본 파일 전송 및 최초 업로드 속도 측면에서도 MP3가 훨씬 가볍고 지연 시간이 없습니다.\n\n따라서 쾌적하고 안정적인 영상 편집을 진행하기 위해 음악 파일은 가볍고 품질이 우수한 고음질 .mp3 파일로 가공하여 작업해 주시기 바랍니다."
    },
    ...
    {
      category: "troubleshoot",
      question: "구글 애드센스 연동 및 자동 광고가 정상적으로 활성화되었는지 어떻게 확인하나요?",
      answer: "애드센스 ID 등록 및 배포가 완료된 후 두 가지 방법으로 연동 성공 여부를 직접 검증해 보실 수 있습니다. 첫째, 본인의 도메인 주소 뒤에 /ads.txt 를 입력하여 접속(예: https://domain.com/ads.txt)했을 때 'google.com, pub-XXXXXXXXXXXXXXXX...' 와 같이 메모장 형식의 텍스트가 깨짐 없이 정상 출력되는지 확인합니다. 둘째, 블로그 메인 화면에서 마우스 우클릭 후 [페이지 소스 보기]를 클릭하여 'adsbygoogle' 또는 본인의 'pub-ID'를 검색했을 때 헤더 영역에 애드센스 공식 스크립트 태그가 심겨 있는지 확인합니다. 두 가지가 정상이라면 연동이 성공적으로 마친 것이며, 구글 엔진 분석에 따라 수시간에서 최대 48시간 이내에 자동으로 광고 송출이 개시됩니다."
    },
    {
      category: "troubleshoot",
      question: "블로그 관리 메뉴의 'PageSpeed Insights 성능 측정'은 어떤 기능이며 어떻게 사용하나요?",
      answer: "PageSpeed Insights 성능 측정은 Google Lighthouse 엔진을 사용하여 본인의 개인 브랜드 블로그의 첫 로딩 속도, 모바일/데스크톱 접근성, SEO 최적화 지수 등을 실시간으로 원격 진단해 주는 고급 기능입니다. 블로그 주소(반드시 인터넷에 연결된 공개 실주소)를 입력하고 측정 모드(데스크톱 또는 휴대전화)를 고른 뒤 '성능 측정'을 누르시면 약 10~15초 후 분야별 최적화 점수와 핵심 로딩 지표(LCP, FCP 등)가 도넛 차트로 아름답게 출력됩니다. 로컬 환경 주소(localhost)는 구글 서버가 접속할 수 없어 측정이 불가하니 유의해 주세요."
    }
  ];
