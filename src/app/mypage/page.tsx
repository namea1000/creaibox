...
export default function MyPage() {
  ...
  const checkBrandIdAvailability = async (value: string) => {
    ...
    // ✅ 정적 상수(13K) + DB reserved_brand_ids 하이브리드 체크 (서버사이드 처리)
    const checkRes = await fetch(`/api/brands/check?brand_id=${encodeURIComponent(brand)}`);
    const checkData = await checkRes.json();

    if (checkData.blocked) {
      setBrandAvailable(false);
      alert(checkData.message ?? "❌ 시스템 예약어 또는 사용이 금지된 브랜드 ID입니다.");
      return;
    }
