...
import { GraduationCap, Sparkles, CheckCircle2, ArrowRight, Check } from "lucide-react";
    ...
    <div className="max-w-7xl mx-auto px-6 sm:px-8 py-16 space-y-24">
      ...
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-2 border-b border-neutral-800 pb-4">
          <div>
            <span className="text-xs font-mono text-cyan-400 font-bold uppercase">TARGETED PROGRAMS</span>
              ...
              {EDUCATION_TABLE.map((row, idx) => (
                <tr key={idx} className="hover:bg-neutral-900/40 transition-colors">
                  <td className="py-4 px-6 font-bold text-white whitespace-nowrap">
                    ...
                    <span className="inline-block mt-1 px-2 py-0.5 rounded bg-cyan-500/10 text-cyan-400 text-[10px] font-mono border border-cyan-500/20">
                      {row.badge}
                    </span>
                  </td>
                  <td className="py-4 px-6 leading-relaxed">{row.content}</td>
                  <td className="py-4 px-6 font-mono text-cyan-400 whitespace-nowrap">{row.period}</td>
      ...
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-2 border-b border-neutral-800 pb-4">
          <div>
            <span className="text-xs font-mono text-cyan-400 font-bold uppercase">MAKER EXPERIENCE</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {EXPERIENTIAL_PROGRAMS.map((prog, idx) => (
            <div
              key={idx}
              className="p-6 rounded-2xl bg-[#141414] border border-neutral-800 hover:border-cyan-500/60 transition-all space-y-3 shadow-xl flex flex-col justify-between"
            >
              ...
              <div className="pt-3 border-t border-neutral-800 text-[11px] text-neutral-400 flex items-center justify-between">
                ...
                <span className="text-cyan-400 font-bold">체험 예약 접수 중</span>
              </div>
            </div>
          ))}
        </div>
